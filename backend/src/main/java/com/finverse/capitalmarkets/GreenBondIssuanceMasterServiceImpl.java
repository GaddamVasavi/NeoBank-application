...
/**
 * Transactional Service Implementation for GreenBondIssuanceMaster
 */
@Service
@RequiredArgsConstructor
public class GreenBondIssuanceMasterServiceImpl implements GreenBondIssuanceMasterService {
    private final GreenBondIssuanceMasterRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<GreenBondIssuanceMasterDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<GreenBondIssuanceMasterDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public GreenBondIssuanceMasterDto create(GreenBondIssuanceMasterDto dto) {
        ...
        GreenBondIssuanceMaster entity = GreenBondIssuanceMaster.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public GreenBondIssuanceMasterDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        GreenBondIssuanceMaster entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<GreenBondIssuanceMasterDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private GreenBondIssuanceMasterDto mapToDto(GreenBondIssuanceMaster e) {
        return GreenBondIssuanceMasterDto.builder()
            .id(e.getId())
