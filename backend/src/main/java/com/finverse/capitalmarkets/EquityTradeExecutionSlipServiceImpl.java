...
/**
 * Transactional Service Implementation for EquityTradeExecutionSlip
 */
@Service
@RequiredArgsConstructor
public class EquityTradeExecutionSlipServiceImpl implements EquityTradeExecutionSlipService {
    private final EquityTradeExecutionSlipRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<EquityTradeExecutionSlipDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<EquityTradeExecutionSlipDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public EquityTradeExecutionSlipDto create(EquityTradeExecutionSlipDto dto) {
        ...
        EquityTradeExecutionSlip entity = EquityTradeExecutionSlip.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public EquityTradeExecutionSlipDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        EquityTradeExecutionSlip entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<EquityTradeExecutionSlipDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private EquityTradeExecutionSlipDto mapToDto(EquityTradeExecutionSlip e) {
        return EquityTradeExecutionSlipDto.builder()
            .id(e.getId())
