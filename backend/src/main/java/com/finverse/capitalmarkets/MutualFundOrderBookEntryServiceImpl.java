...
/**
 * Transactional Service Implementation for MutualFundOrderBookEntry
 */
@Service
@RequiredArgsConstructor
public class MutualFundOrderBookEntryServiceImpl implements MutualFundOrderBookEntryService {
    private final MutualFundOrderBookEntryRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<MutualFundOrderBookEntryDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<MutualFundOrderBookEntryDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public MutualFundOrderBookEntryDto create(MutualFundOrderBookEntryDto dto) {
        ...
        MutualFundOrderBookEntry entity = MutualFundOrderBookEntry.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public MutualFundOrderBookEntryDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        MutualFundOrderBookEntry entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<MutualFundOrderBookEntryDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private MutualFundOrderBookEntryDto mapToDto(MutualFundOrderBookEntry e) {
        return MutualFundOrderBookEntryDto.builder()
            .id(e.getId())
