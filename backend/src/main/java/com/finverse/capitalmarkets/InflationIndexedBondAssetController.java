...
/**
 * REST API Controller for InflationIndexedBondAsset
 */
@Tag(name = "Capitalmarkets - InflationIndexedBondAsset")
@RestController
@RequestMapping("/api/v1/capitalmarkets/modules/inflationindexedbondasset")
...
public class InflationIndexedBondAssetController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<InflationIndexedBondAssetDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<InflationIndexedBondAssetDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<InflationIndexedBondAssetDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<InflationIndexedBondAssetDto>> create(@Valid @RequestBody InflationIndexedBondAssetDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<InflationIndexedBondAssetDto>> getByRef(@PathVariable String ref) {
