...
/**
 * Transactional Service Implementation for ZeroKnowledgeProofSolvency
 */
@Service
@RequiredArgsConstructor
public class ZeroKnowledgeProofSolvencyServiceImpl implements ZeroKnowledgeProofSolvencyService {
    private final ZeroKnowledgeProofSolvencyRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<ZeroKnowledgeProofSolvencyDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<ZeroKnowledgeProofSolvencyDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public ZeroKnowledgeProofSolvencyDto create(ZeroKnowledgeProofSolvencyDto dto) {
        ...
        ZeroKnowledgeProofSolvency entity = ZeroKnowledgeProofSolvency.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public ZeroKnowledgeProofSolvencyDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        ZeroKnowledgeProofSolvency entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<ZeroKnowledgeProofSolvencyDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private ZeroKnowledgeProofSolvencyDto mapToDto(ZeroKnowledgeProofSolvency e) {
        return ZeroKnowledgeProofSolvencyDto.builder()
            .id(e.getId())
