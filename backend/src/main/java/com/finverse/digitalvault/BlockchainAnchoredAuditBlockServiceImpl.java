...
/**
 * Transactional Service Implementation for BlockchainAnchoredAuditBlock
 */
@Service
@RequiredArgsConstructor
public class BlockchainAnchoredAuditBlockServiceImpl implements BlockchainAnchoredAuditBlockService {
    private final BlockchainAnchoredAuditBlockRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<BlockchainAnchoredAuditBlockDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<BlockchainAnchoredAuditBlockDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public BlockchainAnchoredAuditBlockDto create(BlockchainAnchoredAuditBlockDto dto) {
        ...
        BlockchainAnchoredAuditBlock entity = BlockchainAnchoredAuditBlock.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public BlockchainAnchoredAuditBlockDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        BlockchainAnchoredAuditBlock entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<BlockchainAnchoredAuditBlockDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private BlockchainAnchoredAuditBlockDto mapToDto(BlockchainAnchoredAuditBlock e) {
        return BlockchainAnchoredAuditBlockDto.builder()
            .id(e.getId())
