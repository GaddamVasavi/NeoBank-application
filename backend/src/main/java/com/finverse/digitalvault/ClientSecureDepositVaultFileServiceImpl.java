...
/**
 * Transactional Service Implementation for ClientSecureDepositVaultFile
 */
@Service
@RequiredArgsConstructor
public class ClientSecureDepositVaultFileServiceImpl implements ClientSecureDepositVaultFileService {
    private final ClientSecureDepositVaultFileRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<ClientSecureDepositVaultFileDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<ClientSecureDepositVaultFileDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public ClientSecureDepositVaultFileDto create(ClientSecureDepositVaultFileDto dto) {
        ...
        ClientSecureDepositVaultFile entity = ClientSecureDepositVaultFile.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public ClientSecureDepositVaultFileDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        ClientSecureDepositVaultFile entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<ClientSecureDepositVaultFileDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private ClientSecureDepositVaultFileDto mapToDto(ClientSecureDepositVaultFile e) {
        return ClientSecureDepositVaultFileDto.builder()
            .id(e.getId())
