...
/**
 * Transactional Service Implementation for MpcKeyShamirSecretShare
 */
@Service
@RequiredArgsConstructor
public class MpcKeyShamirSecretShareServiceImpl implements MpcKeyShamirSecretShareService {
    private final MpcKeyShamirSecretShareRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<MpcKeyShamirSecretShareDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<MpcKeyShamirSecretShareDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public MpcKeyShamirSecretShareDto create(MpcKeyShamirSecretShareDto dto) {
        ...
        MpcKeyShamirSecretShare entity = MpcKeyShamirSecretShare.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public MpcKeyShamirSecretShareDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        MpcKeyShamirSecretShare entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<MpcKeyShamirSecretShareDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private MpcKeyShamirSecretShareDto mapToDto(MpcKeyShamirSecretShare e) {
        return MpcKeyShamirSecretShareDto.builder()
            .id(e.getId())
