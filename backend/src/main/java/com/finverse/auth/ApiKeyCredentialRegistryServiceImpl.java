...
/**
 * Transactional Service Implementation for ApiKeyCredentialRegistry
 */
@Service
@RequiredArgsConstructor
public class ApiKeyCredentialRegistryServiceImpl implements ApiKeyCredentialRegistryService {
    private final ApiKeyCredentialRegistryRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<ApiKeyCredentialRegistryDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<ApiKeyCredentialRegistryDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public ApiKeyCredentialRegistryDto create(ApiKeyCredentialRegistryDto dto) {
        ...
        ApiKeyCredentialRegistry entity = ApiKeyCredentialRegistry.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public ApiKeyCredentialRegistryDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        ApiKeyCredentialRegistry entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<ApiKeyCredentialRegistryDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private ApiKeyCredentialRegistryDto mapToDto(ApiKeyCredentialRegistry e) {
        return ApiKeyCredentialRegistryDto.builder()
            .id(e.getId())
