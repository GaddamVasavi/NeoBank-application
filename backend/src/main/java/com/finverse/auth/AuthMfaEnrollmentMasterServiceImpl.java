...
/**
 * Transactional Service Implementation for AuthMfaEnrollmentMaster
 */
@Service
@RequiredArgsConstructor
public class AuthMfaEnrollmentMasterServiceImpl implements AuthMfaEnrollmentMasterService {
    private final AuthMfaEnrollmentMasterRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<AuthMfaEnrollmentMasterDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<AuthMfaEnrollmentMasterDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public AuthMfaEnrollmentMasterDto create(AuthMfaEnrollmentMasterDto dto) {
        ...
        AuthMfaEnrollmentMaster entity = AuthMfaEnrollmentMaster.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public AuthMfaEnrollmentMasterDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        AuthMfaEnrollmentMaster entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<AuthMfaEnrollmentMasterDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private AuthMfaEnrollmentMasterDto mapToDto(AuthMfaEnrollmentMaster e) {
        return AuthMfaEnrollmentMasterDto.builder()
            .id(e.getId())
