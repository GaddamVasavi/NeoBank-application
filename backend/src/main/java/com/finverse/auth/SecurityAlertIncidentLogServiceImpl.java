...
/**
 * Transactional Service Implementation for SecurityAlertIncidentLog
 */
@Service
@RequiredArgsConstructor
public class SecurityAlertIncidentLogServiceImpl implements SecurityAlertIncidentLogService {
    private final SecurityAlertIncidentLogRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<SecurityAlertIncidentLogDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<SecurityAlertIncidentLogDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public SecurityAlertIncidentLogDto create(SecurityAlertIncidentLogDto dto) {
        ...
        SecurityAlertIncidentLog entity = SecurityAlertIncidentLog.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public SecurityAlertIncidentLogDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        SecurityAlertIncidentLog entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<SecurityAlertIncidentLogDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private SecurityAlertIncidentLogDto mapToDto(SecurityAlertIncidentLog e) {
        return SecurityAlertIncidentLogDto.builder()
            .id(e.getId())
