...
/**
 * Transactional Service Implementation for DeviceFingerprintProfile
 */
@Service
@RequiredArgsConstructor
public class DeviceFingerprintProfileServiceImpl implements DeviceFingerprintProfileService {
    private final DeviceFingerprintProfileRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<DeviceFingerprintProfileDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<DeviceFingerprintProfileDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public DeviceFingerprintProfileDto create(DeviceFingerprintProfileDto dto) {
        ...
        DeviceFingerprintProfile entity = DeviceFingerprintProfile.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public DeviceFingerprintProfileDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        DeviceFingerprintProfile entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<DeviceFingerprintProfileDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private DeviceFingerprintProfileDto mapToDto(DeviceFingerprintProfile e) {
        return DeviceFingerprintProfileDto.builder()
            .id(e.getId())
