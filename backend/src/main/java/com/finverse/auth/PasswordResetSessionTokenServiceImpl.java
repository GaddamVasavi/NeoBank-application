...
/**
 * Transactional Service Implementation for PasswordResetSessionToken
 */
@Service
@RequiredArgsConstructor
public class PasswordResetSessionTokenServiceImpl implements PasswordResetSessionTokenService {
    private final PasswordResetSessionTokenRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<PasswordResetSessionTokenDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<PasswordResetSessionTokenDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public PasswordResetSessionTokenDto create(PasswordResetSessionTokenDto dto) {
        ...
        PasswordResetSessionToken entity = PasswordResetSessionToken.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public PasswordResetSessionTokenDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        PasswordResetSessionToken entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<PasswordResetSessionTokenDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private PasswordResetSessionTokenDto mapToDto(PasswordResetSessionToken e) {
        return PasswordResetSessionTokenDto.builder()
            .id(e.getId())
