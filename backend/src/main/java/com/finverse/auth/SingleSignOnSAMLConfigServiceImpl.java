...
/**
 * Transactional Service Implementation for SingleSignOnSAMLConfig
 */
@Service
@RequiredArgsConstructor
public class SingleSignOnSAMLConfigServiceImpl implements SingleSignOnSAMLConfigService {
    private final SingleSignOnSAMLConfigRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<SingleSignOnSAMLConfigDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<SingleSignOnSAMLConfigDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public SingleSignOnSAMLConfigDto create(SingleSignOnSAMLConfigDto dto) {
        ...
        SingleSignOnSAMLConfig entity = SingleSignOnSAMLConfig.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public SingleSignOnSAMLConfigDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        SingleSignOnSAMLConfig entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<SingleSignOnSAMLConfigDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private SingleSignOnSAMLConfigDto mapToDto(SingleSignOnSAMLConfig e) {
        return SingleSignOnSAMLConfigDto.builder()
            .id(e.getId())
