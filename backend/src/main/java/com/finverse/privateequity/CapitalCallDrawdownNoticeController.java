...
/**
 * REST API Controller for CapitalCallDrawdownNotice
 */
@Tag(name = "Privateequity - CapitalCallDrawdownNotice")
@RestController
@RequestMapping("/api/v1/privateequity/modules/capitalcalldrawdownnotice")
...
public class CapitalCallDrawdownNoticeController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<CapitalCallDrawdownNoticeDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<CapitalCallDrawdownNoticeDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<CapitalCallDrawdownNoticeDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<CapitalCallDrawdownNoticeDto>> create(@Valid @RequestBody CapitalCallDrawdownNoticeDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<CapitalCallDrawdownNoticeDto>> getByRef(@PathVariable String ref) {
