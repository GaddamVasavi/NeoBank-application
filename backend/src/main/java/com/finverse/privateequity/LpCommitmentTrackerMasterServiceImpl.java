...
/**
 * Transactional Service Implementation for LpCommitmentTrackerMaster
 */
@Service
@RequiredArgsConstructor
public class LpCommitmentTrackerMasterServiceImpl implements LpCommitmentTrackerMasterService {
    private final LpCommitmentTrackerMasterRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<LpCommitmentTrackerMasterDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<LpCommitmentTrackerMasterDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public LpCommitmentTrackerMasterDto create(LpCommitmentTrackerMasterDto dto) {
        ...
        LpCommitmentTrackerMaster entity = LpCommitmentTrackerMaster.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public LpCommitmentTrackerMasterDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        LpCommitmentTrackerMaster entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<LpCommitmentTrackerMasterDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private LpCommitmentTrackerMasterDto mapToDto(LpCommitmentTrackerMaster e) {
        return LpCommitmentTrackerMasterDto.builder()
            .id(e.getId())
