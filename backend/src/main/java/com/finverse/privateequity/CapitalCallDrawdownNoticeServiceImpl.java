...
/**
 * Transactional Service Implementation for CapitalCallDrawdownNotice
 */
@Service
@RequiredArgsConstructor
public class CapitalCallDrawdownNoticeServiceImpl implements CapitalCallDrawdownNoticeService {
    private final CapitalCallDrawdownNoticeRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CapitalCallDrawdownNoticeDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CapitalCallDrawdownNoticeDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CapitalCallDrawdownNoticeDto create(CapitalCallDrawdownNoticeDto dto) {
        ...
        CapitalCallDrawdownNotice entity = CapitalCallDrawdownNotice.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CapitalCallDrawdownNoticeDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CapitalCallDrawdownNotice entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CapitalCallDrawdownNoticeDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CapitalCallDrawdownNoticeDto mapToDto(CapitalCallDrawdownNotice e) {
        return CapitalCallDrawdownNoticeDto.builder()
            .id(e.getId())
