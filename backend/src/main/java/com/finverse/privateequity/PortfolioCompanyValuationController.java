...
/**
 * REST API Controller for PortfolioCompanyValuation
 */
@Tag(name = "Privateequity - PortfolioCompanyValuation")
@RestController
@RequestMapping("/api/v1/privateequity/modules/portfoliocompanyvaluation")
...
public class PortfolioCompanyValuationController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<PortfolioCompanyValuationDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<PortfolioCompanyValuationDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<PortfolioCompanyValuationDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<PortfolioCompanyValuationDto>> create(@Valid @RequestBody PortfolioCompanyValuationDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<PortfolioCompanyValuationDto>> getByRef(@PathVariable String ref) {
