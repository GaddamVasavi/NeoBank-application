...
/**
 * Transactional Service Implementation for PortfolioCompanyValuation
 */
@Service
@RequiredArgsConstructor
public class PortfolioCompanyValuationServiceImpl implements PortfolioCompanyValuationService {
    private final PortfolioCompanyValuationRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<PortfolioCompanyValuationDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<PortfolioCompanyValuationDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public PortfolioCompanyValuationDto create(PortfolioCompanyValuationDto dto) {
        ...
        PortfolioCompanyValuation entity = PortfolioCompanyValuation.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public PortfolioCompanyValuationDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        PortfolioCompanyValuation entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<PortfolioCompanyValuationDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private PortfolioCompanyValuationDto mapToDto(PortfolioCompanyValuation e) {
        return PortfolioCompanyValuationDto.builder()
            .id(e.getId())
