...
/**
 * Transactional Service Implementation for TamperEvidentLogVerifier
 */
@Service
@RequiredArgsConstructor
public class TamperEvidentLogVerifierServiceImpl implements TamperEvidentLogVerifierService {
    private final TamperEvidentLogVerifierRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<TamperEvidentLogVerifierDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<TamperEvidentLogVerifierDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public TamperEvidentLogVerifierDto create(TamperEvidentLogVerifierDto dto) {
        ...
        TamperEvidentLogVerifier entity = TamperEvidentLogVerifier.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public TamperEvidentLogVerifierDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        TamperEvidentLogVerifier entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<TamperEvidentLogVerifierDto> listAll() {
