...
/**
 * Transactional Service Implementation for RegulatoryInspectorSession
 */
@Service
@RequiredArgsConstructor
public class RegulatoryInspectorSessionServiceImpl implements RegulatoryInspectorSessionService {
    private final RegulatoryInspectorSessionRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<RegulatoryInspectorSessionDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<RegulatoryInspectorSessionDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public RegulatoryInspectorSessionDto create(RegulatoryInspectorSessionDto dto) {
        ...
        RegulatoryInspectorSession entity = RegulatoryInspectorSession.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public RegulatoryInspectorSessionDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        RegulatoryInspectorSession entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<RegulatoryInspectorSessionDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private RegulatoryInspectorSessionDto mapToDto(RegulatoryInspectorSession e) {
        return RegulatoryInspectorSessionDto.builder()
            .id(e.getId())
