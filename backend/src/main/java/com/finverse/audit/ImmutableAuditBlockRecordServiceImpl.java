...
/**
 * Transactional Service Implementation for ImmutableAuditBlockRecord
 */
@Service
@RequiredArgsConstructor
public class ImmutableAuditBlockRecordServiceImpl implements ImmutableAuditBlockRecordService {
    private final ImmutableAuditBlockRecordRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<ImmutableAuditBlockRecordDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<ImmutableAuditBlockRecordDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public ImmutableAuditBlockRecordDto create(ImmutableAuditBlockRecordDto dto) {
        ...
        ImmutableAuditBlockRecord entity = ImmutableAuditBlockRecord.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public ImmutableAuditBlockRecordDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        ImmutableAuditBlockRecord entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<ImmutableAuditBlockRecordDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private ImmutableAuditBlockRecordDto mapToDto(ImmutableAuditBlockRecord e) {
        return ImmutableAuditBlockRecordDto.builder()
            .id(e.getId())
