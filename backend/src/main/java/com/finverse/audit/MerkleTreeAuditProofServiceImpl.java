...
/**
 * Transactional Service Implementation for MerkleTreeAuditProof
 */
@Service
@RequiredArgsConstructor
public class MerkleTreeAuditProofServiceImpl implements MerkleTreeAuditProofService {
    private final MerkleTreeAuditProofRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<MerkleTreeAuditProofDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<MerkleTreeAuditProofDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public MerkleTreeAuditProofDto create(MerkleTreeAuditProofDto dto) {
        ...
        MerkleTreeAuditProof entity = MerkleTreeAuditProof.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public MerkleTreeAuditProofDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        MerkleTreeAuditProof entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<MerkleTreeAuditProofDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private MerkleTreeAuditProofDto mapToDto(MerkleTreeAuditProof e) {
        return MerkleTreeAuditProofDto.builder()
            .id(e.getId())
