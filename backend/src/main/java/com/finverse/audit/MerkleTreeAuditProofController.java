...
/**
 * REST API Controller for MerkleTreeAuditProof
 */
@Tag(name = "Audit - MerkleTreeAuditProof")
@RestController
@RequestMapping("/api/v1/audit/modules/merkletreeauditproof")
@RequiredArgsConstructor
public class MerkleTreeAuditProofController {
    private final MerkleTreeAuditProofService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<MerkleTreeAuditProofDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<MerkleTreeAuditProofDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<MerkleTreeAuditProofDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<MerkleTreeAuditProofDto>> create(@Valid @RequestBody MerkleTreeAuditProofDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<MerkleTreeAuditProofDto>> getByRef(@PathVariable String ref) {
