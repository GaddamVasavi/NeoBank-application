...
/**
 * REST API Controller for FxSpotOrderExecution
 */
@Tag(name = "Treasury - FxSpotOrderExecution")
@RestController
@RequestMapping("/api/v1/treasury/modules/fxspotorderexecution")
@RequiredArgsConstructor
public class FxSpotOrderExecutionController {
    private final FxSpotOrderExecutionService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<FxSpotOrderExecutionDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<FxSpotOrderExecutionDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<FxSpotOrderExecutionDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<FxSpotOrderExecutionDto>> create(@Valid @RequestBody FxSpotOrderExecutionDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<FxSpotOrderExecutionDto>> getByRef(@PathVariable String ref) {
