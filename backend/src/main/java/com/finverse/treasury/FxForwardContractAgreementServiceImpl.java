...
/**
 * Transactional Service Implementation for FxForwardContractAgreement
 */
@Service
@RequiredArgsConstructor
public class FxForwardContractAgreementServiceImpl implements FxForwardContractAgreementService {
    private final FxForwardContractAgreementRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<FxForwardContractAgreementDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<FxForwardContractAgreementDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public FxForwardContractAgreementDto create(FxForwardContractAgreementDto dto) {
        ...
        FxForwardContractAgreement entity = FxForwardContractAgreement.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public FxForwardContractAgreementDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        FxForwardContractAgreement entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<FxForwardContractAgreementDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private FxForwardContractAgreementDto mapToDto(FxForwardContractAgreement e) {
        return FxForwardContractAgreementDto.builder()
            .id(e.getId())
