...
/**
 * Transactional Service Implementation for InterestRateSwapContract
 */
@Service
@RequiredArgsConstructor
public class InterestRateSwapContractServiceImpl implements InterestRateSwapContractService {
    private final InterestRateSwapContractRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<InterestRateSwapContractDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<InterestRateSwapContractDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public InterestRateSwapContractDto create(InterestRateSwapContractDto dto) {
        ...
        InterestRateSwapContract entity = InterestRateSwapContract.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public InterestRateSwapContractDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        InterestRateSwapContract entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<InterestRateSwapContractDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private InterestRateSwapContractDto mapToDto(InterestRateSwapContract e) {
        return InterestRateSwapContractDto.builder()
            .id(e.getId())
