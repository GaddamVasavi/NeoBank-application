...
/**
 * REST API Controller for FxForwardContractAgreement
 */
@Tag(name = "Treasury - FxForwardContractAgreement")
@RestController
@RequestMapping("/api/v1/treasury/modules/fxforwardcontractagreement")
...
public class FxForwardContractAgreementController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<FxForwardContractAgreementDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<FxForwardContractAgreementDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<FxForwardContractAgreementDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<FxForwardContractAgreementDto>> create(@Valid @RequestBody FxForwardContractAgreementDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<FxForwardContractAgreementDto>> getByRef(@PathVariable String ref) {
