...
/**
 * Transactional Service Implementation for CashPoolingSweepStructure
 */
@Service
@RequiredArgsConstructor
public class CashPoolingSweepStructureServiceImpl implements CashPoolingSweepStructureService {
    private final CashPoolingSweepStructureRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CashPoolingSweepStructureDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CashPoolingSweepStructureDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CashPoolingSweepStructureDto create(CashPoolingSweepStructureDto dto) {
        ...
        CashPoolingSweepStructure entity = CashPoolingSweepStructure.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CashPoolingSweepStructureDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CashPoolingSweepStructure entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CashPoolingSweepStructureDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CashPoolingSweepStructureDto mapToDto(CashPoolingSweepStructure e) {
        return CashPoolingSweepStructureDto.builder()
            .id(e.getId())
