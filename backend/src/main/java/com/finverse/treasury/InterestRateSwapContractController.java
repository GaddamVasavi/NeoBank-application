...
/**
 * REST API Controller for InterestRateSwapContract
 */
@Tag(name = "Treasury - InterestRateSwapContract")
@RestController
@RequestMapping("/api/v1/treasury/modules/interestrateswapcontract")
...
public class InterestRateSwapContractController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<InterestRateSwapContractDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<InterestRateSwapContractDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<InterestRateSwapContractDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<InterestRateSwapContractDto>> create(@Valid @RequestBody InterestRateSwapContractDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<InterestRateSwapContractDto>> getByRef(@PathVariable String ref) {
