...
/**
 * Transactional Service Implementation for NonDeliverableForwardDeal
 */
@Service
@RequiredArgsConstructor
public class NonDeliverableForwardDealServiceImpl implements NonDeliverableForwardDealService {
    private final NonDeliverableForwardDealRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<NonDeliverableForwardDealDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<NonDeliverableForwardDealDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public NonDeliverableForwardDealDto create(NonDeliverableForwardDealDto dto) {
        ...
        NonDeliverableForwardDeal entity = NonDeliverableForwardDeal.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public NonDeliverableForwardDealDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        NonDeliverableForwardDeal entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<NonDeliverableForwardDealDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private NonDeliverableForwardDealDto mapToDto(NonDeliverableForwardDeal e) {
        return NonDeliverableForwardDealDto.builder()
            .id(e.getId())
