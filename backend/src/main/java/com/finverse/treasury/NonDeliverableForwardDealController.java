...
/**
 * REST API Controller for NonDeliverableForwardDeal
 */
@Tag(name = "Treasury - NonDeliverableForwardDeal")
@RestController
@RequestMapping("/api/v1/treasury/modules/nondeliverableforwarddeal")
...
public class NonDeliverableForwardDealController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<NonDeliverableForwardDealDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<NonDeliverableForwardDealDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<NonDeliverableForwardDealDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<NonDeliverableForwardDealDto>> create(@Valid @RequestBody NonDeliverableForwardDealDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<NonDeliverableForwardDealDto>> getByRef(@PathVariable String ref) {
