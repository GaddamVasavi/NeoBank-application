...
/**
 * Transactional Service Implementation for TrustedBeneficiaryProfile
 */
@Service
@RequiredArgsConstructor
public class TrustedBeneficiaryProfileServiceImpl implements TrustedBeneficiaryProfileService {
    private final TrustedBeneficiaryProfileRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<TrustedBeneficiaryProfileDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<TrustedBeneficiaryProfileDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public TrustedBeneficiaryProfileDto create(TrustedBeneficiaryProfileDto dto) {
        ...
        TrustedBeneficiaryProfile entity = TrustedBeneficiaryProfile.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public TrustedBeneficiaryProfileDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        TrustedBeneficiaryProfile entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<TrustedBeneficiaryProfileDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private TrustedBeneficiaryProfileDto mapToDto(TrustedBeneficiaryProfile e) {
        return TrustedBeneficiaryProfileDto.builder()
            .id(e.getId())
