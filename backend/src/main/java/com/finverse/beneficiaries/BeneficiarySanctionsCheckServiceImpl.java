...
/**
 * Transactional Service Implementation for BeneficiarySanctionsCheck
 */
@Service
@RequiredArgsConstructor
public class BeneficiarySanctionsCheckServiceImpl implements BeneficiarySanctionsCheckService {
    private final BeneficiarySanctionsCheckRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<BeneficiarySanctionsCheckDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<BeneficiarySanctionsCheckDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public BeneficiarySanctionsCheckDto create(BeneficiarySanctionsCheckDto dto) {
        ...
        BeneficiarySanctionsCheck entity = BeneficiarySanctionsCheck.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public BeneficiarySanctionsCheckDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        BeneficiarySanctionsCheck entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<BeneficiarySanctionsCheckDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private BeneficiarySanctionsCheckDto mapToDto(BeneficiarySanctionsCheck e) {
        return BeneficiarySanctionsCheckDto.builder()
            .id(e.getId())
