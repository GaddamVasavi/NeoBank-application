...
/**
 * Transactional Service Implementation for IbanValidationRegistry
 */
@Service
@RequiredArgsConstructor
public class IbanValidationRegistryServiceImpl implements IbanValidationRegistryService {
    private final IbanValidationRegistryRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<IbanValidationRegistryDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<IbanValidationRegistryDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public IbanValidationRegistryDto create(IbanValidationRegistryDto dto) {
        ...
        IbanValidationRegistry entity = IbanValidationRegistry.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public IbanValidationRegistryDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        IbanValidationRegistry entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<IbanValidationRegistryDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private IbanValidationRegistryDto mapToDto(IbanValidationRegistry e) {
        return IbanValidationRegistryDto.builder()
            .id(e.getId())
