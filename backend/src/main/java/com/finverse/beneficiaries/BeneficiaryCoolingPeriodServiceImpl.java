...
/**
 * Transactional Service Implementation for BeneficiaryCoolingPeriod
 */
@Service
@RequiredArgsConstructor
public class BeneficiaryCoolingPeriodServiceImpl implements BeneficiaryCoolingPeriodService {
    private final BeneficiaryCoolingPeriodRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<BeneficiaryCoolingPeriodDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<BeneficiaryCoolingPeriodDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public BeneficiaryCoolingPeriodDto create(BeneficiaryCoolingPeriodDto dto) {
        ...
        BeneficiaryCoolingPeriod entity = BeneficiaryCoolingPeriod.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public BeneficiaryCoolingPeriodDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        BeneficiaryCoolingPeriod entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<BeneficiaryCoolingPeriodDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private BeneficiaryCoolingPeriodDto mapToDto(BeneficiaryCoolingPeriod e) {
        return BeneficiaryCoolingPeriodDto.builder()
            .id(e.getId())
