...
/**
 * Transactional Service Implementation for BulkPayoutBatchUpload
 */
@Service
@RequiredArgsConstructor
public class BulkPayoutBatchUploadServiceImpl implements BulkPayoutBatchUploadService {
    private final BulkPayoutBatchUploadRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<BulkPayoutBatchUploadDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<BulkPayoutBatchUploadDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public BulkPayoutBatchUploadDto create(BulkPayoutBatchUploadDto dto) {
        ...
        BulkPayoutBatchUpload entity = BulkPayoutBatchUpload.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public BulkPayoutBatchUploadDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        BulkPayoutBatchUpload entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<BulkPayoutBatchUploadDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private BulkPayoutBatchUploadDto mapToDto(BulkPayoutBatchUpload e) {
        return BulkPayoutBatchUploadDto.builder()
            .id(e.getId())
