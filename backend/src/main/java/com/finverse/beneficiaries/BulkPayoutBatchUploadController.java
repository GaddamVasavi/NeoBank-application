...
/**
 * REST API Controller for BulkPayoutBatchUpload
 */
@Tag(name = "Beneficiaries - BulkPayoutBatchUpload")
@RestController
@RequestMapping("/api/v1/beneficiaries/modules/bulkpayoutbatchupload")
@RequiredArgsConstructor
public class BulkPayoutBatchUploadController {
    private final BulkPayoutBatchUploadService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<BulkPayoutBatchUploadDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<BulkPayoutBatchUploadDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<BulkPayoutBatchUploadDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<BulkPayoutBatchUploadDto>> create(@Valid @RequestBody BulkPayoutBatchUploadDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<BulkPayoutBatchUploadDto>> getByRef(@PathVariable String ref) {
