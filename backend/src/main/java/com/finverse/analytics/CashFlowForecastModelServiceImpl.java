...
/**
 * Transactional Service Implementation for CashFlowForecastModel
 */
@Service
@RequiredArgsConstructor
public class CashFlowForecastModelServiceImpl implements CashFlowForecastModelService {
    private final CashFlowForecastModelRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CashFlowForecastModelDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CashFlowForecastModelDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CashFlowForecastModelDto create(CashFlowForecastModelDto dto) {
        ...
        CashFlowForecastModel entity = CashFlowForecastModel.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CashFlowForecastModelDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CashFlowForecastModel entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CashFlowForecastModelDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CashFlowForecastModelDto mapToDto(CashFlowForecastModel e) {
        return CashFlowForecastModelDto.builder()
            .id(e.getId())
