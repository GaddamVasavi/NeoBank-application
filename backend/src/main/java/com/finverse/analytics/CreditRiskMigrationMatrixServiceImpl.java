...
/**
 * Transactional Service Implementation for CreditRiskMigrationMatrix
 */
@Service
@RequiredArgsConstructor
public class CreditRiskMigrationMatrixServiceImpl implements CreditRiskMigrationMatrixService {
    private final CreditRiskMigrationMatrixRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CreditRiskMigrationMatrixDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CreditRiskMigrationMatrixDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CreditRiskMigrationMatrixDto create(CreditRiskMigrationMatrixDto dto) {
        ...
        CreditRiskMigrationMatrix entity = CreditRiskMigrationMatrix.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CreditRiskMigrationMatrixDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CreditRiskMigrationMatrix entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CreditRiskMigrationMatrixDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CreditRiskMigrationMatrixDto mapToDto(CreditRiskMigrationMatrix e) {
        return CreditRiskMigrationMatrixDto.builder()
            .id(e.getId())
