...
/**
 * Transactional Service Implementation for CustomerLifetimeValueModel
 */
@Service
@RequiredArgsConstructor
public class CustomerLifetimeValueModelServiceImpl implements CustomerLifetimeValueModelService {
    private final CustomerLifetimeValueModelRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CustomerLifetimeValueModelDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CustomerLifetimeValueModelDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CustomerLifetimeValueModelDto create(CustomerLifetimeValueModelDto dto) {
        ...
        CustomerLifetimeValueModel entity = CustomerLifetimeValueModel.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CustomerLifetimeValueModelDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CustomerLifetimeValueModel entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CustomerLifetimeValueModelDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CustomerLifetimeValueModelDto mapToDto(CustomerLifetimeValueModel e) {
        return CustomerLifetimeValueModelDto.builder()
            .id(e.getId())
