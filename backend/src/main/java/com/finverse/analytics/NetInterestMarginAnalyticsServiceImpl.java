...
/**
 * Transactional Service Implementation for NetInterestMarginAnalytics
 */
@Service
@RequiredArgsConstructor
public class NetInterestMarginAnalyticsServiceImpl implements NetInterestMarginAnalyticsService {
    private final NetInterestMarginAnalyticsRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<NetInterestMarginAnalyticsDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<NetInterestMarginAnalyticsDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public NetInterestMarginAnalyticsDto create(NetInterestMarginAnalyticsDto dto) {
        ...
        NetInterestMarginAnalytics entity = NetInterestMarginAnalytics.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public NetInterestMarginAnalyticsDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        NetInterestMarginAnalytics entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<NetInterestMarginAnalyticsDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private NetInterestMarginAnalyticsDto mapToDto(NetInterestMarginAnalytics e) {
        return NetInterestMarginAnalyticsDto.builder()
            .id(e.getId())
