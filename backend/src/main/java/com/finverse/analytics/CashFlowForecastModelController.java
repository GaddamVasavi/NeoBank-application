...
/**
 * REST API Controller for CashFlowForecastModel
 */
@Tag(name = "Analytics - CashFlowForecastModel")
@RestController
@RequestMapping("/api/v1/analytics/modules/cashflowforecastmodel")
@RequiredArgsConstructor
public class CashFlowForecastModelController {
    private final CashFlowForecastModelService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<CashFlowForecastModelDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<CashFlowForecastModelDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<CashFlowForecastModelDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<CashFlowForecastModelDto>> create(@Valid @RequestBody CashFlowForecastModelDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<CashFlowForecastModelDto>> getByRef(@PathVariable String ref) {
