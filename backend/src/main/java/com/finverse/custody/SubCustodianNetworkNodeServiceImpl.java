...
/**
 * Transactional Service Implementation for SubCustodianNetworkNode
 */
@Service
@RequiredArgsConstructor
public class SubCustodianNetworkNodeServiceImpl implements SubCustodianNetworkNodeService {
    private final SubCustodianNetworkNodeRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<SubCustodianNetworkNodeDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<SubCustodianNetworkNodeDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public SubCustodianNetworkNodeDto create(SubCustodianNetworkNodeDto dto) {
        ...
        SubCustodianNetworkNode entity = SubCustodianNetworkNode.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public SubCustodianNetworkNodeDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        SubCustodianNetworkNode entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<SubCustodianNetworkNodeDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private SubCustodianNetworkNodeDto mapToDto(SubCustodianNetworkNode e) {
        return SubCustodianNetworkNodeDto.builder()
            .id(e.getId())
