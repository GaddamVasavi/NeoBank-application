...
/**
 * Transactional Service Implementation for CorporateActionProxyVote
 */
@Service
@RequiredArgsConstructor
public class CorporateActionProxyVoteServiceImpl implements CorporateActionProxyVoteService {
    private final CorporateActionProxyVoteRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CorporateActionProxyVoteDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CorporateActionProxyVoteDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CorporateActionProxyVoteDto create(CorporateActionProxyVoteDto dto) {
        ...
        CorporateActionProxyVote entity = CorporateActionProxyVote.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CorporateActionProxyVoteDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CorporateActionProxyVote entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CorporateActionProxyVoteDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CorporateActionProxyVoteDto mapToDto(CorporateActionProxyVote e) {
        return CorporateActionProxyVoteDto.builder()
            .id(e.getId())
