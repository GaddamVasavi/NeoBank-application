...
/**
 * REST API Controller for SubCustodianNetworkNode
 */
@Tag(name = "Custody - SubCustodianNetworkNode")
@RestController
@RequestMapping("/api/v1/custody/modules/subcustodiannetworknode")
...
public class SubCustodianNetworkNodeController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<SubCustodianNetworkNodeDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<SubCustodianNetworkNodeDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<SubCustodianNetworkNodeDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<SubCustodianNetworkNodeDto>> create(@Valid @RequestBody SubCustodianNetworkNodeDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<SubCustodianNetworkNodeDto>> getByRef(@PathVariable String ref) {
