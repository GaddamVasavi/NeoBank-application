...
/**
 * Transactional Service Implementation for DividendReinvestmentPlan
 */
@Service
@RequiredArgsConstructor
public class DividendReinvestmentPlanServiceImpl implements DividendReinvestmentPlanService {
    private final DividendReinvestmentPlanRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<DividendReinvestmentPlanDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<DividendReinvestmentPlanDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public DividendReinvestmentPlanDto create(DividendReinvestmentPlanDto dto) {
        ...
        DividendReinvestmentPlan entity = DividendReinvestmentPlan.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public DividendReinvestmentPlanDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        DividendReinvestmentPlan entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<DividendReinvestmentPlanDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private DividendReinvestmentPlanDto mapToDto(DividendReinvestmentPlan e) {
        return DividendReinvestmentPlanDto.builder()
            .id(e.getId())
