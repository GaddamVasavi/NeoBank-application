...
/**
 * Transactional Service Implementation for SegregatedCustodyVaultAccount
 */
@Service
@RequiredArgsConstructor
public class SegregatedCustodyVaultAccountServiceImpl implements SegregatedCustodyVaultAccountService {
    private final SegregatedCustodyVaultAccountRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<SegregatedCustodyVaultAccountDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<SegregatedCustodyVaultAccountDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public SegregatedCustodyVaultAccountDto create(SegregatedCustodyVaultAccountDto dto) {
        ...
        SegregatedCustodyVaultAccount entity = SegregatedCustodyVaultAccount.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public SegregatedCustodyVaultAccountDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        SegregatedCustodyVaultAccount entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<SegregatedCustodyVaultAccountDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private SegregatedCustodyVaultAccountDto mapToDto(SegregatedCustodyVaultAccount e) {
        return SegregatedCustodyVaultAccountDto.builder()
            .id(e.getId())
