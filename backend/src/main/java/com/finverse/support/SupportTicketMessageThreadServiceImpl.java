...
/**
 * Transactional Service Implementation for SupportTicketMessageThread
 */
@Service
@RequiredArgsConstructor
public class SupportTicketMessageThreadServiceImpl implements SupportTicketMessageThreadService {
    private final SupportTicketMessageThreadRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<SupportTicketMessageThreadDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<SupportTicketMessageThreadDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public SupportTicketMessageThreadDto create(SupportTicketMessageThreadDto dto) {
        ...
        SupportTicketMessageThread entity = SupportTicketMessageThread.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public SupportTicketMessageThreadDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        SupportTicketMessageThread entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<SupportTicketMessageThreadDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private SupportTicketMessageThreadDto mapToDto(SupportTicketMessageThread e) {
        return SupportTicketMessageThreadDto.builder()
            .id(e.getId())
