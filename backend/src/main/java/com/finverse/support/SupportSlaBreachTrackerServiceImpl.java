...
/**
 * Transactional Service Implementation for SupportSlaBreachTracker
 */
@Service
@RequiredArgsConstructor
public class SupportSlaBreachTrackerServiceImpl implements SupportSlaBreachTrackerService {
    private final SupportSlaBreachTrackerRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<SupportSlaBreachTrackerDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<SupportSlaBreachTrackerDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public SupportSlaBreachTrackerDto create(SupportSlaBreachTrackerDto dto) {
        ...
        SupportSlaBreachTracker entity = SupportSlaBreachTracker.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public SupportSlaBreachTrackerDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        SupportSlaBreachTracker entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<SupportSlaBreachTrackerDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private SupportSlaBreachTrackerDto mapToDto(SupportSlaBreachTracker e) {
        return SupportSlaBreachTrackerDto.builder()
            .id(e.getId())
