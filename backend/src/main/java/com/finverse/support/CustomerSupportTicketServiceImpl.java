...
/**
 * Transactional Service Implementation for CustomerSupportTicket
 */
@Service
@RequiredArgsConstructor
public class CustomerSupportTicketServiceImpl implements CustomerSupportTicketService {
    private final CustomerSupportTicketRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CustomerSupportTicketDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CustomerSupportTicketDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CustomerSupportTicketDto create(CustomerSupportTicketDto dto) {
        ...
        CustomerSupportTicket entity = CustomerSupportTicket.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CustomerSupportTicketDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CustomerSupportTicket entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CustomerSupportTicketDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CustomerSupportTicketDto mapToDto(CustomerSupportTicket e) {
        return CustomerSupportTicketDto.builder()
            .id(e.getId())
