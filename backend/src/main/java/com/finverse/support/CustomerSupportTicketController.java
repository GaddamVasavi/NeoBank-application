...
/**
 * REST API Controller for CustomerSupportTicket
 */
@Tag(name = "Support - CustomerSupportTicket")
@RestController
@RequestMapping("/api/v1/support/modules/customersupportticket")
@RequiredArgsConstructor
public class CustomerSupportTicketController {
    private final CustomerSupportTicketService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<CustomerSupportTicketDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<CustomerSupportTicketDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<CustomerSupportTicketDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<CustomerSupportTicketDto>> create(@Valid @RequestBody CustomerSupportTicketDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<CustomerSupportTicketDto>> getByRef(@PathVariable String ref) {
