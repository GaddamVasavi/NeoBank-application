...
/**
 * REST API Controller for KycReviewerDecisionLog
 */
@Tag(name = "Kyc - KycReviewerDecisionLog")
@RestController
@RequestMapping("/api/v1/kyc/modules/kycreviewerdecisionlog")
@RequiredArgsConstructor
public class KycReviewerDecisionLogController {
    private final KycReviewerDecisionLogService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<KycReviewerDecisionLogDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<KycReviewerDecisionLogDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<KycReviewerDecisionLogDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<KycReviewerDecisionLogDto>> create(@Valid @RequestBody KycReviewerDecisionLogDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<KycReviewerDecisionLogDto>> getByRef(@PathVariable String ref) {
