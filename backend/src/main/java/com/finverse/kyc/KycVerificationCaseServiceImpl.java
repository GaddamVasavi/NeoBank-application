...
/**
 * Transactional Service Implementation for KycVerificationCase
 */
@Service
@RequiredArgsConstructor
public class KycVerificationCaseServiceImpl implements KycVerificationCaseService {
    private final KycVerificationCaseRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<KycVerificationCaseDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<KycVerificationCaseDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public KycVerificationCaseDto create(KycVerificationCaseDto dto) {
        ...
        KycVerificationCase entity = KycVerificationCase.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public KycVerificationCaseDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        KycVerificationCase entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<KycVerificationCaseDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private KycVerificationCaseDto mapToDto(KycVerificationCase e) {
        return KycVerificationCaseDto.builder()
            .id(e.getId())
