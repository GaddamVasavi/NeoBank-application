...
/**
 * Transactional Service Implementation for ProofOfAddressDocument
 */
@Service
@RequiredArgsConstructor
public class ProofOfAddressDocumentServiceImpl implements ProofOfAddressDocumentService {
    private final ProofOfAddressDocumentRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<ProofOfAddressDocumentDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<ProofOfAddressDocumentDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public ProofOfAddressDocumentDto create(ProofOfAddressDocumentDto dto) {
        ...
        ProofOfAddressDocument entity = ProofOfAddressDocument.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public ProofOfAddressDocumentDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        ProofOfAddressDocument entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<ProofOfAddressDocumentDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private ProofOfAddressDocumentDto mapToDto(ProofOfAddressDocument e) {
        return ProofOfAddressDocumentDto.builder()
            .id(e.getId())
