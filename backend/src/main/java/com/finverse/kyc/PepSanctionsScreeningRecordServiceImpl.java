...
/**
 * Transactional Service Implementation for PepSanctionsScreeningRecord
 */
@Service
@RequiredArgsConstructor
public class PepSanctionsScreeningRecordServiceImpl implements PepSanctionsScreeningRecordService {
    private final PepSanctionsScreeningRecordRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<PepSanctionsScreeningRecordDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<PepSanctionsScreeningRecordDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public PepSanctionsScreeningRecordDto create(PepSanctionsScreeningRecordDto dto) {
        ...
        PepSanctionsScreeningRecord entity = PepSanctionsScreeningRecord.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public PepSanctionsScreeningRecordDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        PepSanctionsScreeningRecord entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<PepSanctionsScreeningRecordDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private PepSanctionsScreeningRecordDto mapToDto(PepSanctionsScreeningRecord e) {
        return PepSanctionsScreeningRecordDto.builder()
            .id(e.getId())
