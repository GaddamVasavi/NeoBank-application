...
/**
 * REST API Controller for BiometricIdentityMatch
 */
@Tag(name = "Kyc - BiometricIdentityMatch")
@RestController
@RequestMapping("/api/v1/kyc/modules/biometricidentitymatch")
@RequiredArgsConstructor
public class BiometricIdentityMatchController {
    private final BiometricIdentityMatchService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<BiometricIdentityMatchDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<BiometricIdentityMatchDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<BiometricIdentityMatchDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<BiometricIdentityMatchDto>> create(@Valid @RequestBody BiometricIdentityMatchDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<BiometricIdentityMatchDto>> getByRef(@PathVariable String ref) {
