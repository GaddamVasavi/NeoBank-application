...
/**
 * Transactional Service Implementation for SourceOfFundsAttestation
 */
@Service
@RequiredArgsConstructor
public class SourceOfFundsAttestationServiceImpl implements SourceOfFundsAttestationService {
    private final SourceOfFundsAttestationRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<SourceOfFundsAttestationDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<SourceOfFundsAttestationDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public SourceOfFundsAttestationDto create(SourceOfFundsAttestationDto dto) {
        ...
        SourceOfFundsAttestation entity = SourceOfFundsAttestation.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public SourceOfFundsAttestationDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        SourceOfFundsAttestation entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<SourceOfFundsAttestationDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private SourceOfFundsAttestationDto mapToDto(SourceOfFundsAttestation e) {
        return SourceOfFundsAttestationDto.builder()
            .id(e.getId())
