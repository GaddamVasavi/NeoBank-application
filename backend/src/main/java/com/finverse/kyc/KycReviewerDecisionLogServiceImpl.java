...
/**
 * Transactional Service Implementation for KycReviewerDecisionLog
 */
@Service
@RequiredArgsConstructor
public class KycReviewerDecisionLogServiceImpl implements KycReviewerDecisionLogService {
    private final KycReviewerDecisionLogRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<KycReviewerDecisionLogDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<KycReviewerDecisionLogDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public KycReviewerDecisionLogDto create(KycReviewerDecisionLogDto dto) {
        ...
        KycReviewerDecisionLog entity = KycReviewerDecisionLog.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public KycReviewerDecisionLogDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        KycReviewerDecisionLog entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<KycReviewerDecisionLogDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private KycReviewerDecisionLogDto mapToDto(KycReviewerDecisionLog e) {
        return KycReviewerDecisionLogDto.builder()
            .id(e.getId())
