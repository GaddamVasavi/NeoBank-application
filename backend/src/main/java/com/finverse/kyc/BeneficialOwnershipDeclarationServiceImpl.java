...
/**
 * Transactional Service Implementation for BeneficialOwnershipDeclaration
 */
@Service
@RequiredArgsConstructor
public class BeneficialOwnershipDeclarationServiceImpl implements BeneficialOwnershipDeclarationService {
    private final BeneficialOwnershipDeclarationRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<BeneficialOwnershipDeclarationDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<BeneficialOwnershipDeclarationDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public BeneficialOwnershipDeclarationDto create(BeneficialOwnershipDeclarationDto dto) {
        ...
        BeneficialOwnershipDeclaration entity = BeneficialOwnershipDeclaration.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public BeneficialOwnershipDeclarationDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        BeneficialOwnershipDeclaration entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<BeneficialOwnershipDeclarationDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private BeneficialOwnershipDeclarationDto mapToDto(BeneficialOwnershipDeclaration e) {
        return BeneficialOwnershipDeclarationDto.builder()
            .id(e.getId())
