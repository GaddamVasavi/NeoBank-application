...
/**
 * REST API Controller for ProofOfAddressDocument
 */
@Tag(name = "Kyc - ProofOfAddressDocument")
@RestController
@RequestMapping("/api/v1/kyc/modules/proofofaddressdocument")
@RequiredArgsConstructor
public class ProofOfAddressDocumentController {
    private final ProofOfAddressDocumentService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<ProofOfAddressDocumentDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<ProofOfAddressDocumentDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<ProofOfAddressDocumentDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<ProofOfAddressDocumentDto>> create(@Valid @RequestBody ProofOfAddressDocumentDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<ProofOfAddressDocumentDto>> getByRef(@PathVariable String ref) {
