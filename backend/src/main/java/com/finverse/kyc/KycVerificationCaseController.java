...
/**
 * REST API Controller for KycVerificationCase
 */
@Tag(name = "Kyc - KycVerificationCase")
@RestController
@RequestMapping("/api/v1/kyc/modules/kycverificationcase")
@RequiredArgsConstructor
public class KycVerificationCaseController {
    private final KycVerificationCaseService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<KycVerificationCaseDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<KycVerificationCaseDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<KycVerificationCaseDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<KycVerificationCaseDto>> create(@Valid @RequestBody KycVerificationCaseDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<KycVerificationCaseDto>> getByRef(@PathVariable String ref) {
