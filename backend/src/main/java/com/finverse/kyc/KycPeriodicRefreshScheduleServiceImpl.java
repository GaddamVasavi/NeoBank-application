...
/**
 * Transactional Service Implementation for KycPeriodicRefreshSchedule
 */
@Service
@RequiredArgsConstructor
public class KycPeriodicRefreshScheduleServiceImpl implements KycPeriodicRefreshScheduleService {
    private final KycPeriodicRefreshScheduleRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<KycPeriodicRefreshScheduleDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<KycPeriodicRefreshScheduleDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public KycPeriodicRefreshScheduleDto create(KycPeriodicRefreshScheduleDto dto) {
        ...
        KycPeriodicRefreshSchedule entity = KycPeriodicRefreshSchedule.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public KycPeriodicRefreshScheduleDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        KycPeriodicRefreshSchedule entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<KycPeriodicRefreshScheduleDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private KycPeriodicRefreshScheduleDto mapToDto(KycPeriodicRefreshSchedule e) {
        return KycPeriodicRefreshScheduleDto.builder()
            .id(e.getId())
