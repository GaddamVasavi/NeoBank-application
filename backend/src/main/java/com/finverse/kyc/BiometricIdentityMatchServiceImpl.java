...
/**
 * Transactional Service Implementation for BiometricIdentityMatch
 */
@Service
@RequiredArgsConstructor
public class BiometricIdentityMatchServiceImpl implements BiometricIdentityMatchService {
    private final BiometricIdentityMatchRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<BiometricIdentityMatchDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<BiometricIdentityMatchDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public BiometricIdentityMatchDto create(BiometricIdentityMatchDto dto) {
        ...
        BiometricIdentityMatch entity = BiometricIdentityMatch.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public BiometricIdentityMatchDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        BiometricIdentityMatch entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<BiometricIdentityMatchDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private BiometricIdentityMatchDto mapToDto(BiometricIdentityMatch e) {
        return BiometricIdentityMatchDto.builder()
            .id(e.getId())
