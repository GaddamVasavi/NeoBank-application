...
/**
 * Transactional Service Implementation for CouponDiscountPromotion
 */
@Service
@RequiredArgsConstructor
public class CouponDiscountPromotionServiceImpl implements CouponDiscountPromotionService {
    private final CouponDiscountPromotionRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CouponDiscountPromotionDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CouponDiscountPromotionDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CouponDiscountPromotionDto create(CouponDiscountPromotionDto dto) {
        ...
        CouponDiscountPromotion entity = CouponDiscountPromotion.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CouponDiscountPromotionDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CouponDiscountPromotion entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CouponDiscountPromotionDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CouponDiscountPromotionDto mapToDto(CouponDiscountPromotion e) {
        return CouponDiscountPromotionDto.builder()
            .id(e.getId())
