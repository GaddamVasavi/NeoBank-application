...
/**
 * Transactional Service Implementation for SaaSSubscriptionContract
 */
@Service
@RequiredArgsConstructor
public class SaaSSubscriptionContractServiceImpl implements SaaSSubscriptionContractService {
    private final SaaSSubscriptionContractRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<SaaSSubscriptionContractDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<SaaSSubscriptionContractDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public SaaSSubscriptionContractDto create(SaaSSubscriptionContractDto dto) {
        ...
        SaaSSubscriptionContract entity = SaaSSubscriptionContract.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public SaaSSubscriptionContractDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        SaaSSubscriptionContract entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<SaaSSubscriptionContractDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private SaaSSubscriptionContractDto mapToDto(SaaSSubscriptionContract e) {
        return SaaSSubscriptionContractDto.builder()
            .id(e.getId())
