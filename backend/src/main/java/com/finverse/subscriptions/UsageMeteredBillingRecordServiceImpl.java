...
/**
 * Transactional Service Implementation for UsageMeteredBillingRecord
 */
@Service
@RequiredArgsConstructor
public class UsageMeteredBillingRecordServiceImpl implements UsageMeteredBillingRecordService {
    private final UsageMeteredBillingRecordRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<UsageMeteredBillingRecordDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<UsageMeteredBillingRecordDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public UsageMeteredBillingRecordDto create(UsageMeteredBillingRecordDto dto) {
        ...
        UsageMeteredBillingRecord entity = UsageMeteredBillingRecord.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public UsageMeteredBillingRecordDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        UsageMeteredBillingRecord entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<UsageMeteredBillingRecordDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private UsageMeteredBillingRecordDto mapToDto(UsageMeteredBillingRecord e) {
        return UsageMeteredBillingRecordDto.builder()
            .id(e.getId())
