...
/**
 * Transactional Service Implementation for ChurnAnalyticsRecord
 */
@Service
@RequiredArgsConstructor
public class ChurnAnalyticsRecordServiceImpl implements ChurnAnalyticsRecordService {
    private final ChurnAnalyticsRecordRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<ChurnAnalyticsRecordDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<ChurnAnalyticsRecordDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public ChurnAnalyticsRecordDto create(ChurnAnalyticsRecordDto dto) {
        ...
        ChurnAnalyticsRecord entity = ChurnAnalyticsRecord.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public ChurnAnalyticsRecordDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        ChurnAnalyticsRecord entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<ChurnAnalyticsRecordDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private ChurnAnalyticsRecordDto mapToDto(ChurnAnalyticsRecord e) {
        return ChurnAnalyticsRecordDto.builder()
            .id(e.getId())
