...
/**
 * Transactional Service Implementation for ProrationAdjustmentLog
 */
@Service
@RequiredArgsConstructor
public class ProrationAdjustmentLogServiceImpl implements ProrationAdjustmentLogService {
    private final ProrationAdjustmentLogRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<ProrationAdjustmentLogDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<ProrationAdjustmentLogDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public ProrationAdjustmentLogDto create(ProrationAdjustmentLogDto dto) {
        ...
        ProrationAdjustmentLog entity = ProrationAdjustmentLog.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public ProrationAdjustmentLogDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        ProrationAdjustmentLog entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<ProrationAdjustmentLogDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private ProrationAdjustmentLogDto mapToDto(ProrationAdjustmentLog e) {
        return ProrationAdjustmentLogDto.builder()
            .id(e.getId())
