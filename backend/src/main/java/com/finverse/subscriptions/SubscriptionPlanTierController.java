...
/**
 * REST API Controller for SubscriptionPlanTier
 */
@Tag(name = "Subscriptions - SubscriptionPlanTier")
@RestController
@RequestMapping("/api/v1/subscriptions/modules/subscriptionplantier")
@RequiredArgsConstructor
public class SubscriptionPlanTierController {
    private final SubscriptionPlanTierService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<SubscriptionPlanTierDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<SubscriptionPlanTierDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<SubscriptionPlanTierDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<SubscriptionPlanTierDto>> create(@Valid @RequestBody SubscriptionPlanTierDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<SubscriptionPlanTierDto>> getByRef(@PathVariable String ref) {
