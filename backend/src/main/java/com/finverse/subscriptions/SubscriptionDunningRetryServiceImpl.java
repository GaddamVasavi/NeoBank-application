...
/**
 * Transactional Service Implementation for SubscriptionDunningRetry
 */
@Service
@RequiredArgsConstructor
public class SubscriptionDunningRetryServiceImpl implements SubscriptionDunningRetryService {
    private final SubscriptionDunningRetryRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<SubscriptionDunningRetryDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<SubscriptionDunningRetryDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public SubscriptionDunningRetryDto create(SubscriptionDunningRetryDto dto) {
        ...
        SubscriptionDunningRetry entity = SubscriptionDunningRetry.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public SubscriptionDunningRetryDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        SubscriptionDunningRetry entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<SubscriptionDunningRetryDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private SubscriptionDunningRetryDto mapToDto(SubscriptionDunningRetry e) {
        return SubscriptionDunningRetryDto.builder()
            .id(e.getId())
