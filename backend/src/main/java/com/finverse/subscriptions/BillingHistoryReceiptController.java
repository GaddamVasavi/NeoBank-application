...
/**
 * REST API Controller for BillingHistoryReceipt
 */
@Tag(name = "Subscriptions - BillingHistoryReceipt")
@RestController
@RequestMapping("/api/v1/subscriptions/modules/billinghistoryreceipt")
@RequiredArgsConstructor
public class BillingHistoryReceiptController {
    private final BillingHistoryReceiptService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<BillingHistoryReceiptDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<BillingHistoryReceiptDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<BillingHistoryReceiptDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<BillingHistoryReceiptDto>> create(@Valid @RequestBody BillingHistoryReceiptDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<BillingHistoryReceiptDto>> getByRef(@PathVariable String ref) {
