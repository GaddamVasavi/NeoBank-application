...
/**
 * Transactional Service Implementation for EnterpriseAddonFeature
 */
@Service
@RequiredArgsConstructor
public class EnterpriseAddonFeatureServiceImpl implements EnterpriseAddonFeatureService {
    private final EnterpriseAddonFeatureRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<EnterpriseAddonFeatureDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<EnterpriseAddonFeatureDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public EnterpriseAddonFeatureDto create(EnterpriseAddonFeatureDto dto) {
        ...
        EnterpriseAddonFeature entity = EnterpriseAddonFeature.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public EnterpriseAddonFeatureDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        EnterpriseAddonFeature entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<EnterpriseAddonFeatureDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private EnterpriseAddonFeatureDto mapToDto(EnterpriseAddonFeature e) {
        return EnterpriseAddonFeatureDto.builder()
            .id(e.getId())
