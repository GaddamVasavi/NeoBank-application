...
/**
 * Transactional Service Implementation for SubscriptionRenewalSchedule
 */
@Service
@RequiredArgsConstructor
public class SubscriptionRenewalScheduleServiceImpl implements SubscriptionRenewalScheduleService {
    private final SubscriptionRenewalScheduleRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<SubscriptionRenewalScheduleDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<SubscriptionRenewalScheduleDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public SubscriptionRenewalScheduleDto create(SubscriptionRenewalScheduleDto dto) {
        ...
        SubscriptionRenewalSchedule entity = SubscriptionRenewalSchedule.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public SubscriptionRenewalScheduleDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        SubscriptionRenewalSchedule entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<SubscriptionRenewalScheduleDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private SubscriptionRenewalScheduleDto mapToDto(SubscriptionRenewalSchedule e) {
        return SubscriptionRenewalScheduleDto.builder()
            .id(e.getId())
