...
/**
 * Transactional Service Implementation for BillingHistoryReceipt
 */
@Service
@RequiredArgsConstructor
public class BillingHistoryReceiptServiceImpl implements BillingHistoryReceiptService {
    private final BillingHistoryReceiptRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<BillingHistoryReceiptDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<BillingHistoryReceiptDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public BillingHistoryReceiptDto create(BillingHistoryReceiptDto dto) {
        ...
        BillingHistoryReceipt entity = BillingHistoryReceipt.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public BillingHistoryReceiptDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        BillingHistoryReceipt entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<BillingHistoryReceiptDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private BillingHistoryReceiptDto mapToDto(BillingHistoryReceipt e) {
        return BillingHistoryReceiptDto.builder()
            .id(e.getId())
