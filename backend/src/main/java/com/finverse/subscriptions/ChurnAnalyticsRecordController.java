...
/**
 * REST API Controller for ChurnAnalyticsRecord
 */
@Tag(name = "Subscriptions - ChurnAnalyticsRecord")
@RestController
@RequestMapping("/api/v1/subscriptions/modules/churnanalyticsrecord")
@RequiredArgsConstructor
public class ChurnAnalyticsRecordController {
    private final ChurnAnalyticsRecordService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<ChurnAnalyticsRecordDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<ChurnAnalyticsRecordDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<ChurnAnalyticsRecordDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<ChurnAnalyticsRecordDto>> create(@Valid @RequestBody ChurnAnalyticsRecordDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<ChurnAnalyticsRecordDto>> getByRef(@PathVariable String ref) {
