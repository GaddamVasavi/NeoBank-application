...
/**
 * Transactional Service Implementation for EmailDispatchQueueItem
 */
@Service
@RequiredArgsConstructor
public class EmailDispatchQueueItemServiceImpl implements EmailDispatchQueueItemService {
    private final EmailDispatchQueueItemRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<EmailDispatchQueueItemDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<EmailDispatchQueueItemDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public EmailDispatchQueueItemDto create(EmailDispatchQueueItemDto dto) {
        ...
        EmailDispatchQueueItem entity = EmailDispatchQueueItem.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public EmailDispatchQueueItemDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        EmailDispatchQueueItem entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<EmailDispatchQueueItemDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private EmailDispatchQueueItemDto mapToDto(EmailDispatchQueueItem e) {
        return EmailDispatchQueueItemDto.builder()
            .id(e.getId())
