...
/**
 * REST API Controller for CustomerNotificationPreference
 */
@Tag(name = "Notifications - CustomerNotificationPreference")
@RestController
@RequestMapping("/api/v1/notifications/modules/customernotificationpreference")
@RequiredArgsConstructor
public class CustomerNotificationPreferenceController {
    private final CustomerNotificationPreferenceService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<CustomerNotificationPreferenceDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<CustomerNotificationPreferenceDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<CustomerNotificationPreferenceDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<CustomerNotificationPreferenceDto>> create(@Valid @RequestBody CustomerNotificationPreferenceDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<CustomerNotificationPreferenceDto>> getByRef(@PathVariable String ref) {
