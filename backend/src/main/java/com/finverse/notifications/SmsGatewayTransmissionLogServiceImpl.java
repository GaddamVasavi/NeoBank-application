...
/**
 * Transactional Service Implementation for SmsGatewayTransmissionLog
 */
@Service
@RequiredArgsConstructor
public class SmsGatewayTransmissionLogServiceImpl implements SmsGatewayTransmissionLogService {
    private final SmsGatewayTransmissionLogRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<SmsGatewayTransmissionLogDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<SmsGatewayTransmissionLogDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public SmsGatewayTransmissionLogDto create(SmsGatewayTransmissionLogDto dto) {
        ...
        SmsGatewayTransmissionLog entity = SmsGatewayTransmissionLog.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public SmsGatewayTransmissionLogDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        SmsGatewayTransmissionLog entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<SmsGatewayTransmissionLogDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private SmsGatewayTransmissionLogDto mapToDto(SmsGatewayTransmissionLog e) {
        return SmsGatewayTransmissionLogDto.builder()
            .id(e.getId())
