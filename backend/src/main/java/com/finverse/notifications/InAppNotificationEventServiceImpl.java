...
/**
 * Transactional Service Implementation for InAppNotificationEvent
 */
@Service
@RequiredArgsConstructor
public class InAppNotificationEventServiceImpl implements InAppNotificationEventService {
    private final InAppNotificationEventRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<InAppNotificationEventDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<InAppNotificationEventDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public InAppNotificationEventDto create(InAppNotificationEventDto dto) {
        ...
        InAppNotificationEvent entity = InAppNotificationEvent.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public InAppNotificationEventDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        InAppNotificationEvent entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<InAppNotificationEventDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private InAppNotificationEventDto mapToDto(InAppNotificationEvent e) {
        return InAppNotificationEventDto.builder()
            .id(e.getId())
