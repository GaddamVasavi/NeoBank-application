...
/**
 * Transactional Service Implementation for NonPerformingLoanWorkout
 */
@Service
@RequiredArgsConstructor
public class NonPerformingLoanWorkoutServiceImpl implements NonPerformingLoanWorkoutService {
    private final NonPerformingLoanWorkoutRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<NonPerformingLoanWorkoutDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<NonPerformingLoanWorkoutDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public NonPerformingLoanWorkoutDto create(NonPerformingLoanWorkoutDto dto) {
        ...
        NonPerformingLoanWorkout entity = NonPerformingLoanWorkout.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public NonPerformingLoanWorkoutDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        NonPerformingLoanWorkout entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<NonPerformingLoanWorkoutDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private NonPerformingLoanWorkoutDto mapToDto(NonPerformingLoanWorkout e) {
        return NonPerformingLoanWorkoutDto.builder()
            .id(e.getId())
