...
/**
 * Transactional Service Implementation for WorkingCapitalCreditLine
 */
@Service
@RequiredArgsConstructor
public class WorkingCapitalCreditLineServiceImpl implements WorkingCapitalCreditLineService {
    private final WorkingCapitalCreditLineRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<WorkingCapitalCreditLineDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<WorkingCapitalCreditLineDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public WorkingCapitalCreditLineDto create(WorkingCapitalCreditLineDto dto) {
        ...
        WorkingCapitalCreditLine entity = WorkingCapitalCreditLine.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public WorkingCapitalCreditLineDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        WorkingCapitalCreditLine entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<WorkingCapitalCreditLineDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private WorkingCapitalCreditLineDto mapToDto(WorkingCapitalCreditLine e) {
        return WorkingCapitalCreditLineDto.builder()
            .id(e.getId())
