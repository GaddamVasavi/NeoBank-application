...
/**
 * Transactional Service Implementation for CollateralPledgeRegistry
 */
@Service
@RequiredArgsConstructor
public class CollateralPledgeRegistryServiceImpl implements CollateralPledgeRegistryService {
    private final CollateralPledgeRegistryRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CollateralPledgeRegistryDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CollateralPledgeRegistryDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CollateralPledgeRegistryDto create(CollateralPledgeRegistryDto dto) {
        ...
        CollateralPledgeRegistry entity = CollateralPledgeRegistry.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CollateralPledgeRegistryDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CollateralPledgeRegistry entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CollateralPledgeRegistryDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CollateralPledgeRegistryDto mapToDto(CollateralPledgeRegistry e) {
        return CollateralPledgeRegistryDto.builder()
            .id(e.getId())
