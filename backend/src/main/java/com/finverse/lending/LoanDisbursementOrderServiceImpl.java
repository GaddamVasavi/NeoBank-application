...
/**
 * Transactional Service Implementation for LoanDisbursementOrder
 */
@Service
@RequiredArgsConstructor
public class LoanDisbursementOrderServiceImpl implements LoanDisbursementOrderService {
    private final LoanDisbursementOrderRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<LoanDisbursementOrderDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<LoanDisbursementOrderDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public LoanDisbursementOrderDto create(LoanDisbursementOrderDto dto) {
        ...
        LoanDisbursementOrder entity = LoanDisbursementOrder.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public LoanDisbursementOrderDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        LoanDisbursementOrder entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<LoanDisbursementOrderDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private LoanDisbursementOrderDto mapToDto(LoanDisbursementOrder e) {
        return LoanDisbursementOrderDto.builder()
            .id(e.getId())
