...
/**
 * Transactional Service Implementation for LoanApplicationDossier
 */
@Service
@RequiredArgsConstructor
public class LoanApplicationDossierServiceImpl implements LoanApplicationDossierService {
    private final LoanApplicationDossierRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<LoanApplicationDossierDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<LoanApplicationDossierDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public LoanApplicationDossierDto create(LoanApplicationDossierDto dto) {
        ...
        LoanApplicationDossier entity = LoanApplicationDossier.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public LoanApplicationDossierDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        LoanApplicationDossier entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<LoanApplicationDossierDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private LoanApplicationDossierDto mapToDto(LoanApplicationDossier e) {
        return LoanApplicationDossierDto.builder()
            .id(e.getId())
