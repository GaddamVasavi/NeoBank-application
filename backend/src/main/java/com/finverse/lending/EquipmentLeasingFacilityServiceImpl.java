...
/**
 * Transactional Service Implementation for EquipmentLeasingFacility
 */
@Service
@RequiredArgsConstructor
public class EquipmentLeasingFacilityServiceImpl implements EquipmentLeasingFacilityService {
    private final EquipmentLeasingFacilityRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<EquipmentLeasingFacilityDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<EquipmentLeasingFacilityDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public EquipmentLeasingFacilityDto create(EquipmentLeasingFacilityDto dto) {
        ...
        EquipmentLeasingFacility entity = EquipmentLeasingFacility.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public EquipmentLeasingFacilityDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        EquipmentLeasingFacility entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<EquipmentLeasingFacilityDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private EquipmentLeasingFacilityDto mapToDto(EquipmentLeasingFacility e) {
        return EquipmentLeasingFacilityDto.builder()
            .id(e.getId())
