...
/**
 * Transactional Service Implementation for EarlyLoanRepaymentSettlement
 */
@Service
@RequiredArgsConstructor
public class EarlyLoanRepaymentSettlementServiceImpl implements EarlyLoanRepaymentSettlementService {
    private final EarlyLoanRepaymentSettlementRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<EarlyLoanRepaymentSettlementDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<EarlyLoanRepaymentSettlementDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public EarlyLoanRepaymentSettlementDto create(EarlyLoanRepaymentSettlementDto dto) {
        ...
        EarlyLoanRepaymentSettlement entity = EarlyLoanRepaymentSettlement.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public EarlyLoanRepaymentSettlementDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        EarlyLoanRepaymentSettlement entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<EarlyLoanRepaymentSettlementDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private EarlyLoanRepaymentSettlementDto mapToDto(EarlyLoanRepaymentSettlement e) {
        return EarlyLoanRepaymentSettlementDto.builder()
            .id(e.getId())
