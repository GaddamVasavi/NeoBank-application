...
/**
 * Transactional Service Implementation for LoanAmortizationSchedule
 */
@Service
@RequiredArgsConstructor
public class LoanAmortizationScheduleServiceImpl implements LoanAmortizationScheduleService {
    private final LoanAmortizationScheduleRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<LoanAmortizationScheduleDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<LoanAmortizationScheduleDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public LoanAmortizationScheduleDto create(LoanAmortizationScheduleDto dto) {
        ...
        LoanAmortizationSchedule entity = LoanAmortizationSchedule.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public LoanAmortizationScheduleDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        LoanAmortizationSchedule entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<LoanAmortizationScheduleDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private LoanAmortizationScheduleDto mapToDto(LoanAmortizationSchedule e) {
        return LoanAmortizationScheduleDto.builder()
            .id(e.getId())
