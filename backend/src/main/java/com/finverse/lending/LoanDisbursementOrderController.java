...
/**
 * REST API Controller for LoanDisbursementOrder
 */
@Tag(name = "Lending - LoanDisbursementOrder")
@RestController
@RequestMapping("/api/v1/lending/modules/loandisbursementorder")
@RequiredArgsConstructor
public class LoanDisbursementOrderController {
    private final LoanDisbursementOrderService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<LoanDisbursementOrderDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<LoanDisbursementOrderDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<LoanDisbursementOrderDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<LoanDisbursementOrderDto>> create(@Valid @RequestBody LoanDisbursementOrderDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<LoanDisbursementOrderDto>> getByRef(@PathVariable String ref) {
