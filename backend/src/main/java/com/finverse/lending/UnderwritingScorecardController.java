...
/**
 * REST API Controller for UnderwritingScorecard
 */
@Tag(name = "Lending - UnderwritingScorecard")
@RestController
@RequestMapping("/api/v1/lending/modules/underwritingscorecard")
@RequiredArgsConstructor
public class UnderwritingScorecardController {
    private final UnderwritingScorecardService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<UnderwritingScorecardDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<UnderwritingScorecardDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<UnderwritingScorecardDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<UnderwritingScorecardDto>> create(@Valid @RequestBody UnderwritingScorecardDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<UnderwritingScorecardDto>> getByRef(@PathVariable String ref) {
