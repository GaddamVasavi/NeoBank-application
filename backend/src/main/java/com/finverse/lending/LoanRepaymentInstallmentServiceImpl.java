...
/**
 * Transactional Service Implementation for LoanRepaymentInstallment
 */
@Service
@RequiredArgsConstructor
public class LoanRepaymentInstallmentServiceImpl implements LoanRepaymentInstallmentService {
    private final LoanRepaymentInstallmentRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<LoanRepaymentInstallmentDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<LoanRepaymentInstallmentDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public LoanRepaymentInstallmentDto create(LoanRepaymentInstallmentDto dto) {
        ...
        LoanRepaymentInstallment entity = LoanRepaymentInstallment.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public LoanRepaymentInstallmentDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        LoanRepaymentInstallment entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<LoanRepaymentInstallmentDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private LoanRepaymentInstallmentDto mapToDto(LoanRepaymentInstallment e) {
        return LoanRepaymentInstallmentDto.builder()
            .id(e.getId())
