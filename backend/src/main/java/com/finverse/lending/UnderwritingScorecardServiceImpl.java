...
/**
 * Transactional Service Implementation for UnderwritingScorecard
 */
@Service
@RequiredArgsConstructor
public class UnderwritingScorecardServiceImpl implements UnderwritingScorecardService {
    private final UnderwritingScorecardRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<UnderwritingScorecardDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<UnderwritingScorecardDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public UnderwritingScorecardDto create(UnderwritingScorecardDto dto) {
        ...
        UnderwritingScorecard entity = UnderwritingScorecard.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public UnderwritingScorecardDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        UnderwritingScorecard entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<UnderwritingScorecardDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private UnderwritingScorecardDto mapToDto(UnderwritingScorecard e) {
        return UnderwritingScorecardDto.builder()
            .id(e.getId())
