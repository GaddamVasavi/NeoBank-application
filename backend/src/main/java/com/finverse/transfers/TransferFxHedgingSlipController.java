...
/**
 * REST API Controller for TransferFxHedgingSlip
 */
@Tag(name = "Transfers - TransferFxHedgingSlip")
@RestController
@RequestMapping("/api/v1/transfers/modules/transferfxhedgingslip")
@RequiredArgsConstructor
public class TransferFxHedgingSlipController {
    private final TransferFxHedgingSlipService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<TransferFxHedgingSlipDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<TransferFxHedgingSlipDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<TransferFxHedgingSlipDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<TransferFxHedgingSlipDto>> create(@Valid @RequestBody TransferFxHedgingSlipDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<TransferFxHedgingSlipDto>> getByRef(@PathVariable String ref) {
