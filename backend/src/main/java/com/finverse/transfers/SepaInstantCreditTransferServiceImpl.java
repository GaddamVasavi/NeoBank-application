...
/**
 * Transactional Service Implementation for SepaInstantCreditTransfer
 */
@Service
@RequiredArgsConstructor
public class SepaInstantCreditTransferServiceImpl implements SepaInstantCreditTransferService {
    private final SepaInstantCreditTransferRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<SepaInstantCreditTransferDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<SepaInstantCreditTransferDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public SepaInstantCreditTransferDto create(SepaInstantCreditTransferDto dto) {
        ...
        SepaInstantCreditTransfer entity = SepaInstantCreditTransfer.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public SepaInstantCreditTransferDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        SepaInstantCreditTransfer entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<SepaInstantCreditTransferDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private SepaInstantCreditTransferDto mapToDto(SepaInstantCreditTransfer e) {
        return SepaInstantCreditTransferDto.builder()
            .id(e.getId())
