...
/**
 * Transactional Service Implementation for TransferFxHedgingSlip
 */
@Service
@RequiredArgsConstructor
public class TransferFxHedgingSlipServiceImpl implements TransferFxHedgingSlipService {
    private final TransferFxHedgingSlipRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<TransferFxHedgingSlipDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<TransferFxHedgingSlipDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public TransferFxHedgingSlipDto create(TransferFxHedgingSlipDto dto) {
        ...
        TransferFxHedgingSlip entity = TransferFxHedgingSlip.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public TransferFxHedgingSlipDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        TransferFxHedgingSlip entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<TransferFxHedgingSlipDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private TransferFxHedgingSlipDto mapToDto(TransferFxHedgingSlip e) {
        return TransferFxHedgingSlipDto.builder()
            .id(e.getId())
