...
/**
 * Transactional Service Implementation for BookTransferTransaction
 */
@Service
@RequiredArgsConstructor
public class BookTransferTransactionServiceImpl implements BookTransferTransactionService {
    private final BookTransferTransactionRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<BookTransferTransactionDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<BookTransferTransactionDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public BookTransferTransactionDto create(BookTransferTransactionDto dto) {
        ...
        BookTransferTransaction entity = BookTransferTransaction.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public BookTransferTransactionDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        BookTransferTransaction entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<BookTransferTransactionDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private BookTransferTransactionDto mapToDto(BookTransferTransaction e) {
        return BookTransferTransactionDto.builder()
            .id(e.getId())
