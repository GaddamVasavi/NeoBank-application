...
/**
 * Transactional Service Implementation for TransferOtpApprovalChallenge
 */
@Service
@RequiredArgsConstructor
public class TransferOtpApprovalChallengeServiceImpl implements TransferOtpApprovalChallengeService {
    private final TransferOtpApprovalChallengeRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<TransferOtpApprovalChallengeDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<TransferOtpApprovalChallengeDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public TransferOtpApprovalChallengeDto create(TransferOtpApprovalChallengeDto dto) {
        ...
        TransferOtpApprovalChallenge entity = TransferOtpApprovalChallenge.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public TransferOtpApprovalChallengeDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        TransferOtpApprovalChallenge entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<TransferOtpApprovalChallengeDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private TransferOtpApprovalChallengeDto mapToDto(TransferOtpApprovalChallenge e) {
        return TransferOtpApprovalChallengeDto.builder()
            .id(e.getId())
