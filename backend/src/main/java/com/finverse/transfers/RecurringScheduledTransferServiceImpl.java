...
/**
 * Transactional Service Implementation for RecurringScheduledTransfer
 */
@Service
@RequiredArgsConstructor
public class RecurringScheduledTransferServiceImpl implements RecurringScheduledTransferService {
    private final RecurringScheduledTransferRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<RecurringScheduledTransferDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<RecurringScheduledTransferDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public RecurringScheduledTransferDto create(RecurringScheduledTransferDto dto) {
        ...
        RecurringScheduledTransfer entity = RecurringScheduledTransfer.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public RecurringScheduledTransferDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        RecurringScheduledTransfer entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<RecurringScheduledTransferDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private RecurringScheduledTransferDto mapToDto(RecurringScheduledTransfer e) {
        return RecurringScheduledTransferDto.builder()
            .id(e.getId())
