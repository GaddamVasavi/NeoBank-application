...
/**
 * REST API Controller for AchNachaBatchTransfer
 */
@Tag(name = "Transfers - AchNachaBatchTransfer")
@RestController
@RequestMapping("/api/v1/transfers/modules/achnachabatchtransfer")
@RequiredArgsConstructor
public class AchNachaBatchTransferController {
    private final AchNachaBatchTransferService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<AchNachaBatchTransferDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<AchNachaBatchTransferDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<AchNachaBatchTransferDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<AchNachaBatchTransferDto>> create(@Valid @RequestBody AchNachaBatchTransferDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<AchNachaBatchTransferDto>> getByRef(@PathVariable String ref) {
