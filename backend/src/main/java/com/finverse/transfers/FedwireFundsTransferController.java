...
/**
 * REST API Controller for FedwireFundsTransfer
 */
@Tag(name = "Transfers - FedwireFundsTransfer")
@RestController
@RequestMapping("/api/v1/transfers/modules/fedwirefundstransfer")
...
public class FedwireFundsTransferController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<FedwireFundsTransferDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<FedwireFundsTransferDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<FedwireFundsTransferDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<FedwireFundsTransferDto>> create(@Valid @RequestBody FedwireFundsTransferDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<FedwireFundsTransferDto>> getByRef(@PathVariable String ref) {
