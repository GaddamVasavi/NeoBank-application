...
/**
 * Transactional Service Implementation for SwiftGpiCrossBorderTransfer
 */
@Service
@RequiredArgsConstructor
public class SwiftGpiCrossBorderTransferServiceImpl implements SwiftGpiCrossBorderTransferService {
    private final SwiftGpiCrossBorderTransferRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<SwiftGpiCrossBorderTransferDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<SwiftGpiCrossBorderTransferDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public SwiftGpiCrossBorderTransferDto create(SwiftGpiCrossBorderTransferDto dto) {
        ...
        SwiftGpiCrossBorderTransfer entity = SwiftGpiCrossBorderTransfer.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public SwiftGpiCrossBorderTransferDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        SwiftGpiCrossBorderTransfer entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<SwiftGpiCrossBorderTransferDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private SwiftGpiCrossBorderTransferDto mapToDto(SwiftGpiCrossBorderTransfer e) {
        return SwiftGpiCrossBorderTransferDto.builder()
            .id(e.getId())
