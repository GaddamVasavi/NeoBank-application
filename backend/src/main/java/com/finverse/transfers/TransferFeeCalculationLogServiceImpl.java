...
/**
 * Transactional Service Implementation for TransferFeeCalculationLog
 */
@Service
@RequiredArgsConstructor
public class TransferFeeCalculationLogServiceImpl implements TransferFeeCalculationLogService {
    private final TransferFeeCalculationLogRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<TransferFeeCalculationLogDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<TransferFeeCalculationLogDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public TransferFeeCalculationLogDto create(TransferFeeCalculationLogDto dto) {
        ...
        TransferFeeCalculationLog entity = TransferFeeCalculationLog.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public TransferFeeCalculationLogDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        TransferFeeCalculationLog entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<TransferFeeCalculationLogDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private TransferFeeCalculationLogDto mapToDto(TransferFeeCalculationLog e) {
        return TransferFeeCalculationLogDto.builder()
            .id(e.getId())
