...
/**
 * Transactional Service Implementation for TransferCancellationDispute
 */
@Service
@RequiredArgsConstructor
public class TransferCancellationDisputeServiceImpl implements TransferCancellationDisputeService {
    private final TransferCancellationDisputeRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<TransferCancellationDisputeDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<TransferCancellationDisputeDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public TransferCancellationDisputeDto create(TransferCancellationDisputeDto dto) {
        ...
        TransferCancellationDispute entity = TransferCancellationDispute.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public TransferCancellationDisputeDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        TransferCancellationDispute entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<TransferCancellationDisputeDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private TransferCancellationDisputeDto mapToDto(TransferCancellationDispute e) {
        return TransferCancellationDisputeDto.builder()
            .id(e.getId())
