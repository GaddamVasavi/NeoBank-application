...
/**
 * Transactional Service Implementation for MarineCargoInsurancePolicy
 */
@Service
@RequiredArgsConstructor
public class MarineCargoInsurancePolicyServiceImpl implements MarineCargoInsurancePolicyService {
    private final MarineCargoInsurancePolicyRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<MarineCargoInsurancePolicyDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<MarineCargoInsurancePolicyDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public MarineCargoInsurancePolicyDto create(MarineCargoInsurancePolicyDto dto) {
        ...
        MarineCargoInsurancePolicy entity = MarineCargoInsurancePolicy.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public MarineCargoInsurancePolicyDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        MarineCargoInsurancePolicy entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<MarineCargoInsurancePolicyDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private MarineCargoInsurancePolicyDto mapToDto(MarineCargoInsurancePolicy e) {
        return MarineCargoInsurancePolicyDto.builder()
            .id(e.getId())
