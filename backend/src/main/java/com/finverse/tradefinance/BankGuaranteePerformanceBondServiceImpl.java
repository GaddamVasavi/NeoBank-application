...
/**
 * Transactional Service Implementation for BankGuaranteePerformanceBond
 */
@Service
@RequiredArgsConstructor
public class BankGuaranteePerformanceBondServiceImpl implements BankGuaranteePerformanceBondService {
    private final BankGuaranteePerformanceBondRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<BankGuaranteePerformanceBondDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<BankGuaranteePerformanceBondDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public BankGuaranteePerformanceBondDto create(BankGuaranteePerformanceBondDto dto) {
        ...
        BankGuaranteePerformanceBond entity = BankGuaranteePerformanceBond.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public BankGuaranteePerformanceBondDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        BankGuaranteePerformanceBond entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<BankGuaranteePerformanceBondDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private BankGuaranteePerformanceBondDto mapToDto(BankGuaranteePerformanceBond e) {
        return BankGuaranteePerformanceBondDto.builder()
            .id(e.getId())
