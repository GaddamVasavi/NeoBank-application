...
/**
 * Transactional Service Implementation for DocumentaryCollectionDraft
 */
@Service
@RequiredArgsConstructor
public class DocumentaryCollectionDraftServiceImpl implements DocumentaryCollectionDraftService {
    private final DocumentaryCollectionDraftRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<DocumentaryCollectionDraftDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<DocumentaryCollectionDraftDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public DocumentaryCollectionDraftDto create(DocumentaryCollectionDraftDto dto) {
        ...
        DocumentaryCollectionDraft entity = DocumentaryCollectionDraft.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public DocumentaryCollectionDraftDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        DocumentaryCollectionDraft entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<DocumentaryCollectionDraftDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private DocumentaryCollectionDraftDto mapToDto(DocumentaryCollectionDraft e) {
        return DocumentaryCollectionDraftDto.builder()
            .id(e.getId())
