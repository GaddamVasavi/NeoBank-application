...
/**
 * Transactional Service Implementation for CommercialLetterOfCredit
 */
@Service
@RequiredArgsConstructor
public class CommercialLetterOfCreditServiceImpl implements CommercialLetterOfCreditService {
    private final CommercialLetterOfCreditRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CommercialLetterOfCreditDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CommercialLetterOfCreditDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CommercialLetterOfCreditDto create(CommercialLetterOfCreditDto dto) {
        ...
        CommercialLetterOfCredit entity = CommercialLetterOfCredit.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CommercialLetterOfCreditDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CommercialLetterOfCredit entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CommercialLetterOfCreditDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CommercialLetterOfCreditDto mapToDto(CommercialLetterOfCredit e) {
        return CommercialLetterOfCreditDto.builder()
            .id(e.getId())
