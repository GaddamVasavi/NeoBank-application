...
/**
 * Transactional Service Implementation for TenantLifecycleManagement
 */
@Service
@RequiredArgsConstructor
public class TenantLifecycleManagementServiceImpl implements TenantLifecycleManagementService {
    private final TenantLifecycleManagementRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<TenantLifecycleManagementDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<TenantLifecycleManagementDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public TenantLifecycleManagementDto create(TenantLifecycleManagementDto dto) {
        ...
        TenantLifecycleManagement entity = TenantLifecycleManagement.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public TenantLifecycleManagementDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        TenantLifecycleManagement entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<TenantLifecycleManagementDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private TenantLifecycleManagementDto mapToDto(TenantLifecycleManagement e) {
        return TenantLifecycleManagementDto.builder()
            .id(e.getId())
