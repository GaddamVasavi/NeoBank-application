...
/**
 * REST API Controller for SuperAdminAccessLog
 */
@Tag(name = "Admin - SuperAdminAccessLog")
@RestController
@RequestMapping("/api/v1/admin/modules/superadminaccesslog")
@RequiredArgsConstructor
public class SuperAdminAccessLogController {
    private final SuperAdminAccessLogService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<SuperAdminAccessLogDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<SuperAdminAccessLogDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<SuperAdminAccessLogDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<SuperAdminAccessLogDto>> create(@Valid @RequestBody SuperAdminAccessLogDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<SuperAdminAccessLogDto>> getByRef(@PathVariable String ref) {
