...
/**
 * Transactional Service Implementation for SuperAdminAccessLog
 */
@Service
@RequiredArgsConstructor
public class SuperAdminAccessLogServiceImpl implements SuperAdminAccessLogService {
    private final SuperAdminAccessLogRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<SuperAdminAccessLogDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<SuperAdminAccessLogDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public SuperAdminAccessLogDto create(SuperAdminAccessLogDto dto) {
        ...
        SuperAdminAccessLog entity = SuperAdminAccessLog.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public SuperAdminAccessLogDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        SuperAdminAccessLog entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<SuperAdminAccessLogDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private SuperAdminAccessLogDto mapToDto(SuperAdminAccessLog e) {
        return SuperAdminAccessLogDto.builder()
            .id(e.getId())
