...
/**
 * Transactional Service Implementation for PlatformTelemetryMetrics
 */
@Service
@RequiredArgsConstructor
public class PlatformTelemetryMetricsServiceImpl implements PlatformTelemetryMetricsService {
    private final PlatformTelemetryMetricsRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<PlatformTelemetryMetricsDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<PlatformTelemetryMetricsDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public PlatformTelemetryMetricsDto create(PlatformTelemetryMetricsDto dto) {
        ...
        PlatformTelemetryMetrics entity = PlatformTelemetryMetrics.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public PlatformTelemetryMetricsDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        PlatformTelemetryMetrics entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<PlatformTelemetryMetricsDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private PlatformTelemetryMetricsDto mapToDto(PlatformTelemetryMetrics e) {
        return PlatformTelemetryMetricsDto.builder()
            .id(e.getId())
