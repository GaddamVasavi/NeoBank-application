...
/**
 * Transactional Service Implementation for GlobalConfigurationParameter
 */
@Service
@RequiredArgsConstructor
public class GlobalConfigurationParameterServiceImpl implements GlobalConfigurationParameterService {
    private final GlobalConfigurationParameterRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<GlobalConfigurationParameterDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<GlobalConfigurationParameterDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public GlobalConfigurationParameterDto create(GlobalConfigurationParameterDto dto) {
        ...
        GlobalConfigurationParameter entity = GlobalConfigurationParameter.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public GlobalConfigurationParameterDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        GlobalConfigurationParameter entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<GlobalConfigurationParameterDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private GlobalConfigurationParameterDto mapToDto(GlobalConfigurationParameter e) {
        return GlobalConfigurationParameterDto.builder()
            .id(e.getId())
