...
/**
 * Transactional Service Implementation for TenantProfileMaster
 */
@Service
@RequiredArgsConstructor
public class TenantProfileMasterServiceImpl implements TenantProfileMasterService {
    private final TenantProfileMasterRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<TenantProfileMasterDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<TenantProfileMasterDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public TenantProfileMasterDto create(TenantProfileMasterDto dto) {
        ...
        TenantProfileMaster entity = TenantProfileMaster.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public TenantProfileMasterDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        TenantProfileMaster entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<TenantProfileMasterDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private TenantProfileMasterDto mapToDto(TenantProfileMaster e) {
        return TenantProfileMasterDto.builder()
            .id(e.getId())
