...
/**
 * REST API Controller for TenantCustomBranding
 */
@Tag(name = "Tenant - TenantCustomBranding")
@RestController
@RequestMapping("/api/v1/tenant/modules/tenantcustombranding")
@RequiredArgsConstructor
public class TenantCustomBrandingController {
    private final TenantCustomBrandingService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<TenantCustomBrandingDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<TenantCustomBrandingDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<TenantCustomBrandingDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<TenantCustomBrandingDto>> create(@Valid @RequestBody TenantCustomBrandingDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<TenantCustomBrandingDto>> getByRef(@PathVariable String ref) {
