...
/**
 * REST API Controller for TenantProfileMaster
 */
@Tag(name = "Tenant - TenantProfileMaster")
@RestController
@RequestMapping("/api/v1/tenant/modules/tenantprofilemaster")
@RequiredArgsConstructor
public class TenantProfileMasterController {
    private final TenantProfileMasterService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<TenantProfileMasterDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<TenantProfileMasterDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<TenantProfileMasterDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<TenantProfileMasterDto>> create(@Valid @RequestBody TenantProfileMasterDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<TenantProfileMasterDto>> getByRef(@PathVariable String ref) {
