...
/**
 * Transactional Service Implementation for TenantBillingAddress
 */
@Service
@RequiredArgsConstructor
public class TenantBillingAddressServiceImpl implements TenantBillingAddressService {
    private final TenantBillingAddressRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<TenantBillingAddressDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<TenantBillingAddressDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public TenantBillingAddressDto create(TenantBillingAddressDto dto) {
        ...
        TenantBillingAddress entity = TenantBillingAddress.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public TenantBillingAddressDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        TenantBillingAddress entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<TenantBillingAddressDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private TenantBillingAddressDto mapToDto(TenantBillingAddress e) {
        return TenantBillingAddressDto.builder()
            .id(e.getId())
