...
/**
 * REST API Controller for TenantFeatureToggle
 */
@Tag(name = "Tenant - TenantFeatureToggle")
@RestController
@RequestMapping("/api/v1/tenant/modules/tenantfeaturetoggle")
@RequiredArgsConstructor
public class TenantFeatureToggleController {
    private final TenantFeatureToggleService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<TenantFeatureToggleDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<TenantFeatureToggleDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<TenantFeatureToggleDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<TenantFeatureToggleDto>> create(@Valid @RequestBody TenantFeatureToggleDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<TenantFeatureToggleDto>> getByRef(@PathVariable String ref) {
