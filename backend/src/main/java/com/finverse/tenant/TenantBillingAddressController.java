...
/**
 * REST API Controller for TenantBillingAddress
 */
@Tag(name = "Tenant - TenantBillingAddress")
@RestController
@RequestMapping("/api/v1/tenant/modules/tenantbillingaddress")
@RequiredArgsConstructor
public class TenantBillingAddressController {
    private final TenantBillingAddressService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<TenantBillingAddressDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<TenantBillingAddressDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<TenantBillingAddressDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<TenantBillingAddressDto>> create(@Valid @RequestBody TenantBillingAddressDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<TenantBillingAddressDto>> getByRef(@PathVariable String ref) {
