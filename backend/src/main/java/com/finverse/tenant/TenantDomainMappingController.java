...
/**
 * REST API Controller for TenantDomainMapping
 */
@Tag(name = "Tenant - TenantDomainMapping")
@RestController
@RequestMapping("/api/v1/tenant/modules/tenantdomainmapping")
@RequiredArgsConstructor
public class TenantDomainMappingController {
    private final TenantDomainMappingService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<TenantDomainMappingDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<TenantDomainMappingDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<TenantDomainMappingDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<TenantDomainMappingDto>> create(@Valid @RequestBody TenantDomainMappingDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<TenantDomainMappingDto>> getByRef(@PathVariable String ref) {
