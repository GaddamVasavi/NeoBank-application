...
/**
 * Transactional Service Implementation for TenantSecurityPolicy
 */
@Service
@RequiredArgsConstructor
public class TenantSecurityPolicyServiceImpl implements TenantSecurityPolicyService {
    private final TenantSecurityPolicyRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<TenantSecurityPolicyDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<TenantSecurityPolicyDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public TenantSecurityPolicyDto create(TenantSecurityPolicyDto dto) {
        ...
        TenantSecurityPolicy entity = TenantSecurityPolicy.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public TenantSecurityPolicyDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        TenantSecurityPolicy entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<TenantSecurityPolicyDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private TenantSecurityPolicyDto mapToDto(TenantSecurityPolicy e) {
        return TenantSecurityPolicyDto.builder()
            .id(e.getId())
