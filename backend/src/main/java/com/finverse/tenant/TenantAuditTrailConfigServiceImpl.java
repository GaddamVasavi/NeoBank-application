...
/**
 * Transactional Service Implementation for TenantAuditTrailConfig
 */
@Service
@RequiredArgsConstructor
public class TenantAuditTrailConfigServiceImpl implements TenantAuditTrailConfigService {
    private final TenantAuditTrailConfigRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<TenantAuditTrailConfigDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<TenantAuditTrailConfigDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public TenantAuditTrailConfigDto create(TenantAuditTrailConfigDto dto) {
        ...
        TenantAuditTrailConfig entity = TenantAuditTrailConfig.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public TenantAuditTrailConfigDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        TenantAuditTrailConfig entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<TenantAuditTrailConfigDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private TenantAuditTrailConfigDto mapToDto(TenantAuditTrailConfig e) {
        return TenantAuditTrailConfigDto.builder()
            .id(e.getId())
