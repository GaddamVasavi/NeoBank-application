...
/**
 * Transactional Service Implementation for TenantFeatureToggle
 */
@Service
@RequiredArgsConstructor
public class TenantFeatureToggleServiceImpl implements TenantFeatureToggleService {
    private final TenantFeatureToggleRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<TenantFeatureToggleDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<TenantFeatureToggleDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public TenantFeatureToggleDto create(TenantFeatureToggleDto dto) {
        ...
        TenantFeatureToggle entity = TenantFeatureToggle.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public TenantFeatureToggleDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        TenantFeatureToggle entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<TenantFeatureToggleDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private TenantFeatureToggleDto mapToDto(TenantFeatureToggle e) {
        return TenantFeatureToggleDto.builder()
            .id(e.getId())
