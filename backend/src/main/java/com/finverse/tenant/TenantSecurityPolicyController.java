...
/**
 * REST API Controller for TenantSecurityPolicy
 */
@Tag(name = "Tenant - TenantSecurityPolicy")
@RestController
@RequestMapping("/api/v1/tenant/modules/tenantsecuritypolicy")
...
public class TenantSecurityPolicyController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<TenantSecurityPolicyDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<TenantSecurityPolicyDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<TenantSecurityPolicyDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<TenantSecurityPolicyDto>> create(@Valid @RequestBody TenantSecurityPolicyDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<TenantSecurityPolicyDto>> getByRef(@PathVariable String ref) {
