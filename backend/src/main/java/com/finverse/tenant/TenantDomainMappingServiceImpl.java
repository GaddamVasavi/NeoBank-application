...
/**
 * Transactional Service Implementation for TenantDomainMapping
 */
@Service
@RequiredArgsConstructor
public class TenantDomainMappingServiceImpl implements TenantDomainMappingService {
    private final TenantDomainMappingRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<TenantDomainMappingDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<TenantDomainMappingDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public TenantDomainMappingDto create(TenantDomainMappingDto dto) {
        ...
        TenantDomainMapping entity = TenantDomainMapping.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public TenantDomainMappingDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        TenantDomainMapping entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<TenantDomainMappingDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private TenantDomainMappingDto mapToDto(TenantDomainMapping e) {
        return TenantDomainMappingDto.builder()
            .id(e.getId())
