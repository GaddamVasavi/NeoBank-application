...
/**
 * Transactional Service Implementation for TenantSubscriptionPlan
 */
@Service
@RequiredArgsConstructor
public class TenantSubscriptionPlanServiceImpl implements TenantSubscriptionPlanService {
    private final TenantSubscriptionPlanRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<TenantSubscriptionPlanDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<TenantSubscriptionPlanDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public TenantSubscriptionPlanDto create(TenantSubscriptionPlanDto dto) {
        ...
        TenantSubscriptionPlan entity = TenantSubscriptionPlan.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public TenantSubscriptionPlanDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        TenantSubscriptionPlan entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<TenantSubscriptionPlanDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private TenantSubscriptionPlanDto mapToDto(TenantSubscriptionPlan e) {
