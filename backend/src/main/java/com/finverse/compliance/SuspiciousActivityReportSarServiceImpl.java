...
/**
 * Transactional Service Implementation for SuspiciousActivityReportSar
 */
@Service
@RequiredArgsConstructor
public class SuspiciousActivityReportSarServiceImpl implements SuspiciousActivityReportSarService {
    private final SuspiciousActivityReportSarRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<SuspiciousActivityReportSarDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<SuspiciousActivityReportSarDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public SuspiciousActivityReportSarDto create(SuspiciousActivityReportSarDto dto) {
        ...
        SuspiciousActivityReportSar entity = SuspiciousActivityReportSar.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public SuspiciousActivityReportSarDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        SuspiciousActivityReportSar entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<SuspiciousActivityReportSarDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private SuspiciousActivityReportSarDto mapToDto(SuspiciousActivityReportSar e) {
        return SuspiciousActivityReportSarDto.builder()
            .id(e.getId())
