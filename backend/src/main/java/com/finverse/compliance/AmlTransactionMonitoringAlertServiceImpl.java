...
/**
 * Transactional Service Implementation for AmlTransactionMonitoringAlert
 */
@Service
@RequiredArgsConstructor
public class AmlTransactionMonitoringAlertServiceImpl implements AmlTransactionMonitoringAlertService {
    private final AmlTransactionMonitoringAlertRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<AmlTransactionMonitoringAlertDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<AmlTransactionMonitoringAlertDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public AmlTransactionMonitoringAlertDto create(AmlTransactionMonitoringAlertDto dto) {
        ...
        AmlTransactionMonitoringAlert entity = AmlTransactionMonitoringAlert.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public AmlTransactionMonitoringAlertDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        AmlTransactionMonitoringAlert entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<AmlTransactionMonitoringAlertDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private AmlTransactionMonitoringAlertDto mapToDto(AmlTransactionMonitoringAlert e) {
        return AmlTransactionMonitoringAlertDto.builder()
            .id(e.getId())
