...
/**
 * Transactional Service Implementation for FatcaCrsTaxWithholdingFile
 */
@Service
@RequiredArgsConstructor
public class FatcaCrsTaxWithholdingFileServiceImpl implements FatcaCrsTaxWithholdingFileService {
    private final FatcaCrsTaxWithholdingFileRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<FatcaCrsTaxWithholdingFileDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<FatcaCrsTaxWithholdingFileDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public FatcaCrsTaxWithholdingFileDto create(FatcaCrsTaxWithholdingFileDto dto) {
        ...
        FatcaCrsTaxWithholdingFile entity = FatcaCrsTaxWithholdingFile.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public FatcaCrsTaxWithholdingFileDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        FatcaCrsTaxWithholdingFile entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<FatcaCrsTaxWithholdingFileDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private FatcaCrsTaxWithholdingFileDto mapToDto(FatcaCrsTaxWithholdingFile e) {
        return FatcaCrsTaxWithholdingFileDto.builder()
            .id(e.getId())
