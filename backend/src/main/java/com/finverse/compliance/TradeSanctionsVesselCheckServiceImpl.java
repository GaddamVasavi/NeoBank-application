...
/**
 * Transactional Service Implementation for TradeSanctionsVesselCheck
 */
@Service
@RequiredArgsConstructor
public class TradeSanctionsVesselCheckServiceImpl implements TradeSanctionsVesselCheckService {
    private final TradeSanctionsVesselCheckRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<TradeSanctionsVesselCheckDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<TradeSanctionsVesselCheckDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public TradeSanctionsVesselCheckDto create(TradeSanctionsVesselCheckDto dto) {
        ...
        TradeSanctionsVesselCheck entity = TradeSanctionsVesselCheck.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public TradeSanctionsVesselCheckDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        TradeSanctionsVesselCheck entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<TradeSanctionsVesselCheckDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private TradeSanctionsVesselCheckDto mapToDto(TradeSanctionsVesselCheck e) {
        return TradeSanctionsVesselCheckDto.builder()
            .id(e.getId())
