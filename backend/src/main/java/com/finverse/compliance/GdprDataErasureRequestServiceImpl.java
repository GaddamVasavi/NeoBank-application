...
/**
 * Transactional Service Implementation for GdprDataErasureRequest
 */
@Service
@RequiredArgsConstructor
public class GdprDataErasureRequestServiceImpl implements GdprDataErasureRequestService {
    private final GdprDataErasureRequestRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<GdprDataErasureRequestDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<GdprDataErasureRequestDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public GdprDataErasureRequestDto create(GdprDataErasureRequestDto dto) {
        ...
        GdprDataErasureRequest entity = GdprDataErasureRequest.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public GdprDataErasureRequestDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        GdprDataErasureRequest entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<GdprDataErasureRequestDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private GdprDataErasureRequestDto mapToDto(GdprDataErasureRequest e) {
        return GdprDataErasureRequestDto.builder()
            .id(e.getId())
