...
/**
 * Transactional Service Implementation for RegulatoryInquiryDossier
 */
@Service
@RequiredArgsConstructor
public class RegulatoryInquiryDossierServiceImpl implements RegulatoryInquiryDossierService {
    private final RegulatoryInquiryDossierRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<RegulatoryInquiryDossierDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<RegulatoryInquiryDossierDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public RegulatoryInquiryDossierDto create(RegulatoryInquiryDossierDto dto) {
        ...
        RegulatoryInquiryDossier entity = RegulatoryInquiryDossier.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public RegulatoryInquiryDossierDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        RegulatoryInquiryDossier entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<RegulatoryInquiryDossierDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private RegulatoryInquiryDossierDto mapToDto(RegulatoryInquiryDossier e) {
        return RegulatoryInquiryDossierDto.builder()
            .id(e.getId())
