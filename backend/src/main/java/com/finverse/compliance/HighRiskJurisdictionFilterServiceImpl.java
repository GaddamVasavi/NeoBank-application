...
/**
 * Transactional Service Implementation for HighRiskJurisdictionFilter
 */
@Service
@RequiredArgsConstructor
public class HighRiskJurisdictionFilterServiceImpl implements HighRiskJurisdictionFilterService {
    private final HighRiskJurisdictionFilterRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<HighRiskJurisdictionFilterDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<HighRiskJurisdictionFilterDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public HighRiskJurisdictionFilterDto create(HighRiskJurisdictionFilterDto dto) {
        ...
        HighRiskJurisdictionFilter entity = HighRiskJurisdictionFilter.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public HighRiskJurisdictionFilterDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        HighRiskJurisdictionFilter entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<HighRiskJurisdictionFilterDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private HighRiskJurisdictionFilterDto mapToDto(HighRiskJurisdictionFilter e) {
        return HighRiskJurisdictionFilterDto.builder()
            .id(e.getId())
