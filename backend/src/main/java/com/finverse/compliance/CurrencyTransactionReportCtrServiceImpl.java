...
/**
 * Transactional Service Implementation for CurrencyTransactionReportCtr
 */
@Service
@RequiredArgsConstructor
public class CurrencyTransactionReportCtrServiceImpl implements CurrencyTransactionReportCtrService {
    private final CurrencyTransactionReportCtrRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CurrencyTransactionReportCtrDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CurrencyTransactionReportCtrDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CurrencyTransactionReportCtrDto create(CurrencyTransactionReportCtrDto dto) {
        ...
        CurrencyTransactionReportCtr entity = CurrencyTransactionReportCtr.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CurrencyTransactionReportCtrDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CurrencyTransactionReportCtr entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CurrencyTransactionReportCtrDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CurrencyTransactionReportCtrDto mapToDto(CurrencyTransactionReportCtr e) {
        return CurrencyTransactionReportCtrDto.builder()
            .id(e.getId())
