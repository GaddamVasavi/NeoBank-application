...
/**
 * Transactional Service Implementation for ContinuousLinkedSettlementPvP
 */
@Service
@RequiredArgsConstructor
public class ContinuousLinkedSettlementPvPServiceImpl implements ContinuousLinkedSettlementPvPService {
    private final ContinuousLinkedSettlementPvPRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<ContinuousLinkedSettlementPvPDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<ContinuousLinkedSettlementPvPDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public ContinuousLinkedSettlementPvPDto create(ContinuousLinkedSettlementPvPDto dto) {
        ...
        ContinuousLinkedSettlementPvP entity = ContinuousLinkedSettlementPvP.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public ContinuousLinkedSettlementPvPDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        ContinuousLinkedSettlementPvP entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<ContinuousLinkedSettlementPvPDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private ContinuousLinkedSettlementPvPDto mapToDto(ContinuousLinkedSettlementPvP e) {
        return ContinuousLinkedSettlementPvPDto.builder()
            .id(e.getId())
