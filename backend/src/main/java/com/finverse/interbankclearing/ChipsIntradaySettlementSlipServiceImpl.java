...
/**
 * Transactional Service Implementation for ChipsIntradaySettlementSlip
 */
@Service
@RequiredArgsConstructor
public class ChipsIntradaySettlementSlipServiceImpl implements ChipsIntradaySettlementSlipService {
    private final ChipsIntradaySettlementSlipRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<ChipsIntradaySettlementSlipDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<ChipsIntradaySettlementSlipDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public ChipsIntradaySettlementSlipDto create(ChipsIntradaySettlementSlipDto dto) {
        ...
        ChipsIntradaySettlementSlip entity = ChipsIntradaySettlementSlip.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public ChipsIntradaySettlementSlipDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        ChipsIntradaySettlementSlip entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<ChipsIntradaySettlementSlipDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private ChipsIntradaySettlementSlipDto mapToDto(ChipsIntradaySettlementSlip e) {
        return ChipsIntradaySettlementSlipDto.builder()
            .id(e.getId())
