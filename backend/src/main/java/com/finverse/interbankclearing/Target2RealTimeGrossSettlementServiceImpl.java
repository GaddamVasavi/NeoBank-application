...
/**
 * Transactional Service Implementation for Target2RealTimeGrossSettlement
 */
@Service
@RequiredArgsConstructor
public class Target2RealTimeGrossSettlementServiceImpl implements Target2RealTimeGrossSettlementService {
    private final Target2RealTimeGrossSettlementRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<Target2RealTimeGrossSettlementDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<Target2RealTimeGrossSettlementDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public Target2RealTimeGrossSettlementDto create(Target2RealTimeGrossSettlementDto dto) {
        ...
        Target2RealTimeGrossSettlement entity = Target2RealTimeGrossSettlement.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public Target2RealTimeGrossSettlementDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        Target2RealTimeGrossSettlement entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<Target2RealTimeGrossSettlementDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private Target2RealTimeGrossSettlementDto mapToDto(Target2RealTimeGrossSettlement e) {
        return Target2RealTimeGrossSettlementDto.builder()
            .id(e.getId())
