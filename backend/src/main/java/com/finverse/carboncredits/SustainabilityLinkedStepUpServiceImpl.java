...
/**
 * Transactional Service Implementation for SustainabilityLinkedStepUp
 */
@Service
@RequiredArgsConstructor
public class SustainabilityLinkedStepUpServiceImpl implements SustainabilityLinkedStepUpService {
    private final SustainabilityLinkedStepUpRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<SustainabilityLinkedStepUpDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<SustainabilityLinkedStepUpDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public SustainabilityLinkedStepUpDto create(SustainabilityLinkedStepUpDto dto) {
        ...
        SustainabilityLinkedStepUp entity = SustainabilityLinkedStepUp.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public SustainabilityLinkedStepUpDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        SustainabilityLinkedStepUp entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<SustainabilityLinkedStepUpDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private SustainabilityLinkedStepUpDto mapToDto(SustainabilityLinkedStepUp e) {
        return SustainabilityLinkedStepUpDto.builder()
            .id(e.getId())
