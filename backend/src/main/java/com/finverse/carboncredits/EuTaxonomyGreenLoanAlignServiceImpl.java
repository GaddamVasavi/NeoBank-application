...
/**
 * Transactional Service Implementation for EuTaxonomyGreenLoanAlign
 */
@Service
@RequiredArgsConstructor
public class EuTaxonomyGreenLoanAlignServiceImpl implements EuTaxonomyGreenLoanAlignService {
    private final EuTaxonomyGreenLoanAlignRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<EuTaxonomyGreenLoanAlignDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<EuTaxonomyGreenLoanAlignDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public EuTaxonomyGreenLoanAlignDto create(EuTaxonomyGreenLoanAlignDto dto) {
        ...
        EuTaxonomyGreenLoanAlign entity = EuTaxonomyGreenLoanAlign.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public EuTaxonomyGreenLoanAlignDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        EuTaxonomyGreenLoanAlign entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<EuTaxonomyGreenLoanAlignDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private EuTaxonomyGreenLoanAlignDto mapToDto(EuTaxonomyGreenLoanAlign e) {
        return EuTaxonomyGreenLoanAlignDto.builder()
            .id(e.getId())
