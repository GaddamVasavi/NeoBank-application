...
/**
 * Transactional Service Implementation for CarbonOffsetRegistrySerial
 */
@Service
@RequiredArgsConstructor
public class CarbonOffsetRegistrySerialServiceImpl implements CarbonOffsetRegistrySerialService {
    private final CarbonOffsetRegistrySerialRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CarbonOffsetRegistrySerialDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CarbonOffsetRegistrySerialDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CarbonOffsetRegistrySerialDto create(CarbonOffsetRegistrySerialDto dto) {
        ...
        CarbonOffsetRegistrySerial entity = CarbonOffsetRegistrySerial.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CarbonOffsetRegistrySerialDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CarbonOffsetRegistrySerial entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CarbonOffsetRegistrySerialDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CarbonOffsetRegistrySerialDto mapToDto(CarbonOffsetRegistrySerial e) {
        return CarbonOffsetRegistrySerialDto.builder()
            .id(e.getId())
