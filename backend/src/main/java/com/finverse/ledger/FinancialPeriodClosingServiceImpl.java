...
/**
 * Transactional Service Implementation for FinancialPeriodClosing
 */
@Service
@RequiredArgsConstructor
public class FinancialPeriodClosingServiceImpl implements FinancialPeriodClosingService {
    private final FinancialPeriodClosingRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<FinancialPeriodClosingDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<FinancialPeriodClosingDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public FinancialPeriodClosingDto create(FinancialPeriodClosingDto dto) {
        ...
        FinancialPeriodClosing entity = FinancialPeriodClosing.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public FinancialPeriodClosingDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        FinancialPeriodClosing entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<FinancialPeriodClosingDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private FinancialPeriodClosingDto mapToDto(FinancialPeriodClosing e) {
        return FinancialPeriodClosingDto.builder()
            .id(e.getId())
