...
/**
 * Transactional Service Implementation for ForeignCurrencyRevaluation
 */
@Service
@RequiredArgsConstructor
public class ForeignCurrencyRevaluationServiceImpl implements ForeignCurrencyRevaluationService {
    private final ForeignCurrencyRevaluationRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<ForeignCurrencyRevaluationDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<ForeignCurrencyRevaluationDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public ForeignCurrencyRevaluationDto create(ForeignCurrencyRevaluationDto dto) {
        ...
        ForeignCurrencyRevaluation entity = ForeignCurrencyRevaluation.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public ForeignCurrencyRevaluationDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        ForeignCurrencyRevaluation entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<ForeignCurrencyRevaluationDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private ForeignCurrencyRevaluationDto mapToDto(ForeignCurrencyRevaluation e) {
        return ForeignCurrencyRevaluationDto.builder()
            .id(e.getId())
