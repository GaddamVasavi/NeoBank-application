...
/**
 * Transactional Service Implementation for ChartOfAccountsMaster
 */
@Service
@RequiredArgsConstructor
public class ChartOfAccountsMasterServiceImpl implements ChartOfAccountsMasterService {
    private final ChartOfAccountsMasterRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<ChartOfAccountsMasterDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<ChartOfAccountsMasterDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public ChartOfAccountsMasterDto create(ChartOfAccountsMasterDto dto) {
        ...
        ChartOfAccountsMaster entity = ChartOfAccountsMaster.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public ChartOfAccountsMasterDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        ChartOfAccountsMaster entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<ChartOfAccountsMasterDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private ChartOfAccountsMasterDto mapToDto(ChartOfAccountsMaster e) {
        return ChartOfAccountsMasterDto.builder()
            .id(e.getId())
