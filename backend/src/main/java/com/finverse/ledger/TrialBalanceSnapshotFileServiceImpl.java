...
/**
 * Transactional Service Implementation for TrialBalanceSnapshotFile
 */
@Service
@RequiredArgsConstructor
public class TrialBalanceSnapshotFileServiceImpl implements TrialBalanceSnapshotFileService {
    private final TrialBalanceSnapshotFileRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<TrialBalanceSnapshotFileDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<TrialBalanceSnapshotFileDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public TrialBalanceSnapshotFileDto create(TrialBalanceSnapshotFileDto dto) {
        ...
        TrialBalanceSnapshotFile entity = TrialBalanceSnapshotFile.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public TrialBalanceSnapshotFileDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        TrialBalanceSnapshotFile entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<TrialBalanceSnapshotFileDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private TrialBalanceSnapshotFileDto mapToDto(TrialBalanceSnapshotFile e) {
        return TrialBalanceSnapshotFileDto.builder()
            .id(e.getId())
