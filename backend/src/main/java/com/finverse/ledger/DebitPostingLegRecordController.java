...
/**
 * REST API Controller for DebitPostingLegRecord
 */
@Tag(name = "Ledger - DebitPostingLegRecord")
@RestController
@RequestMapping("/api/v1/ledger/modules/debitpostinglegrecord")
@RequiredArgsConstructor
public class DebitPostingLegRecordController {
    private final DebitPostingLegRecordService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<DebitPostingLegRecordDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<DebitPostingLegRecordDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<DebitPostingLegRecordDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<DebitPostingLegRecordDto>> create(@Valid @RequestBody DebitPostingLegRecordDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<DebitPostingLegRecordDto>> getByRef(@PathVariable String ref) {
