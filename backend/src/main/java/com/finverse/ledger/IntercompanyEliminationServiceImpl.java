...
/**
 * Transactional Service Implementation for IntercompanyElimination
 */
@Service
@RequiredArgsConstructor
public class IntercompanyEliminationServiceImpl implements IntercompanyEliminationService {
    private final IntercompanyEliminationRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<IntercompanyEliminationDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<IntercompanyEliminationDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public IntercompanyEliminationDto create(IntercompanyEliminationDto dto) {
        ...
        IntercompanyElimination entity = IntercompanyElimination.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public IntercompanyEliminationDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        IntercompanyElimination entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<IntercompanyEliminationDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private IntercompanyEliminationDto mapToDto(IntercompanyElimination e) {
        return IntercompanyEliminationDto.builder()
            .id(e.getId())
