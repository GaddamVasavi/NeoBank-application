...
/**
 * Transactional Service Implementation for DebitPostingLegRecord
 */
@Service
@RequiredArgsConstructor
public class DebitPostingLegRecordServiceImpl implements DebitPostingLegRecordService {
    private final DebitPostingLegRecordRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<DebitPostingLegRecordDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<DebitPostingLegRecordDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public DebitPostingLegRecordDto create(DebitPostingLegRecordDto dto) {
        ...
        DebitPostingLegRecord entity = DebitPostingLegRecord.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public DebitPostingLegRecordDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        DebitPostingLegRecord entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<DebitPostingLegRecordDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private DebitPostingLegRecordDto mapToDto(DebitPostingLegRecord e) {
        return DebitPostingLegRecordDto.builder()
            .id(e.getId())
