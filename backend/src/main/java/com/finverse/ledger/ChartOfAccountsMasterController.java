...
/**
 * REST API Controller for ChartOfAccountsMaster
 */
@Tag(name = "Ledger - ChartOfAccountsMaster")
@RestController
@RequestMapping("/api/v1/ledger/modules/chartofaccountsmaster")
...
public class ChartOfAccountsMasterController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<ChartOfAccountsMasterDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<ChartOfAccountsMasterDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<ChartOfAccountsMasterDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<ChartOfAccountsMasterDto>> create(@Valid @RequestBody ChartOfAccountsMasterDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<ChartOfAccountsMasterDto>> getByRef(@PathVariable String ref) {
