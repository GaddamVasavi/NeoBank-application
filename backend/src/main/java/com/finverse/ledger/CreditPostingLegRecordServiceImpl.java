...
/**
 * Transactional Service Implementation for CreditPostingLegRecord
 */
@Service
@RequiredArgsConstructor
public class CreditPostingLegRecordServiceImpl implements CreditPostingLegRecordService {
    private final CreditPostingLegRecordRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CreditPostingLegRecordDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CreditPostingLegRecordDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CreditPostingLegRecordDto create(CreditPostingLegRecordDto dto) {
        ...
        CreditPostingLegRecord entity = CreditPostingLegRecord.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CreditPostingLegRecordDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CreditPostingLegRecord entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CreditPostingLegRecordDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CreditPostingLegRecordDto mapToDto(CreditPostingLegRecord e) {
        return CreditPostingLegRecordDto.builder()
            .id(e.getId())
