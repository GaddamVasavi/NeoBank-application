...
/**
 * REST API Controller for SuspenseAccountClearance
 */
@Tag(name = "Ledger - SuspenseAccountClearance")
@RestController
@RequestMapping("/api/v1/ledger/modules/suspenseaccountclearance")
...
public class SuspenseAccountClearanceController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<SuspenseAccountClearanceDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<SuspenseAccountClearanceDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<SuspenseAccountClearanceDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<SuspenseAccountClearanceDto>> create(@Valid @RequestBody SuspenseAccountClearanceDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<SuspenseAccountClearanceDto>> getByRef(@PathVariable String ref) {
