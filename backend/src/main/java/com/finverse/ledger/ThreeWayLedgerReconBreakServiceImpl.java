...
/**
 * Transactional Service Implementation for ThreeWayLedgerReconBreak
 */
@Service
@RequiredArgsConstructor
public class ThreeWayLedgerReconBreakServiceImpl implements ThreeWayLedgerReconBreakService {
    private final ThreeWayLedgerReconBreakRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<ThreeWayLedgerReconBreakDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<ThreeWayLedgerReconBreakDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public ThreeWayLedgerReconBreakDto create(ThreeWayLedgerReconBreakDto dto) {
        ...
        ThreeWayLedgerReconBreak entity = ThreeWayLedgerReconBreak.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public ThreeWayLedgerReconBreakDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        ThreeWayLedgerReconBreak entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<ThreeWayLedgerReconBreakDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private ThreeWayLedgerReconBreakDto mapToDto(ThreeWayLedgerReconBreak e) {
        return ThreeWayLedgerReconBreakDto.builder()
            .id(e.getId())
