...
/**
 * Transactional Service Implementation for JournalEntryBatchMaster
 */
@Service
@RequiredArgsConstructor
public class JournalEntryBatchMasterServiceImpl implements JournalEntryBatchMasterService {
    private final JournalEntryBatchMasterRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<JournalEntryBatchMasterDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<JournalEntryBatchMasterDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public JournalEntryBatchMasterDto create(JournalEntryBatchMasterDto dto) {
        ...
        JournalEntryBatchMaster entity = JournalEntryBatchMaster.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public JournalEntryBatchMasterDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        JournalEntryBatchMaster entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<JournalEntryBatchMasterDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private JournalEntryBatchMasterDto mapToDto(JournalEntryBatchMaster e) {
        return JournalEntryBatchMasterDto.builder()
            .id(e.getId())
