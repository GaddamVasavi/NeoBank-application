...
/**
 * Transactional Service Implementation for SuspenseAccountClearance
 */
@Service
@RequiredArgsConstructor
public class SuspenseAccountClearanceServiceImpl implements SuspenseAccountClearanceService {
    private final SuspenseAccountClearanceRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<SuspenseAccountClearanceDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<SuspenseAccountClearanceDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public SuspenseAccountClearanceDto create(SuspenseAccountClearanceDto dto) {
        ...
        SuspenseAccountClearance entity = SuspenseAccountClearance.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public SuspenseAccountClearanceDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        SuspenseAccountClearance entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<SuspenseAccountClearanceDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private SuspenseAccountClearanceDto mapToDto(SuspenseAccountClearance e) {
        return SuspenseAccountClearanceDto.builder()
            .id(e.getId())
