...
/**
 * Transactional Service Implementation for PaymentReceiptDossier
 */
@Service
@RequiredArgsConstructor
public class PaymentReceiptDossierServiceImpl implements PaymentReceiptDossierService {
    private final PaymentReceiptDossierRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<PaymentReceiptDossierDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<PaymentReceiptDossierDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public PaymentReceiptDossierDto create(PaymentReceiptDossierDto dto) {
        ...
        PaymentReceiptDossier entity = PaymentReceiptDossier.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public PaymentReceiptDossierDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        PaymentReceiptDossier entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<PaymentReceiptDossierDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private PaymentReceiptDossierDto mapToDto(PaymentReceiptDossier e) {
        return PaymentReceiptDossierDto.builder()
            .id(e.getId())
