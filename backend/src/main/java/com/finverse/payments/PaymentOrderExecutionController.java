...
/**
 * REST API Controller for PaymentOrderExecution
 */
@Tag(name = "Payments - PaymentOrderExecution")
@RestController
@RequestMapping("/api/v1/payments/modules/paymentorderexecution")
@RequiredArgsConstructor
public class PaymentOrderExecutionController {
    private final PaymentOrderExecutionService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<PaymentOrderExecutionDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<PaymentOrderExecutionDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<PaymentOrderExecutionDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<PaymentOrderExecutionDto>> create(@Valid @RequestBody PaymentOrderExecutionDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<PaymentOrderExecutionDto>> getByRef(@PathVariable String ref) {
