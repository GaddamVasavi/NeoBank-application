...
/**
 * REST API Controller for PaymentReceiptDossier
 */
@Tag(name = "Payments - PaymentReceiptDossier")
@RestController
@RequestMapping("/api/v1/payments/modules/paymentreceiptdossier")
@RequiredArgsConstructor
public class PaymentReceiptDossierController {
    private final PaymentReceiptDossierService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<PaymentReceiptDossierDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<PaymentReceiptDossierDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<PaymentReceiptDossierDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<PaymentReceiptDossierDto>> create(@Valid @RequestBody PaymentReceiptDossierDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<PaymentReceiptDossierDto>> getByRef(@PathVariable String ref) {
