...
/**
 * Transactional Service Implementation for PaymentOrderExecution
 */
@Service
@RequiredArgsConstructor
public class PaymentOrderExecutionServiceImpl implements PaymentOrderExecutionService {
    private final PaymentOrderExecutionRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<PaymentOrderExecutionDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<PaymentOrderExecutionDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public PaymentOrderExecutionDto create(PaymentOrderExecutionDto dto) {
        ...
        PaymentOrderExecution entity = PaymentOrderExecution.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public PaymentOrderExecutionDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        PaymentOrderExecution entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<PaymentOrderExecutionDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private PaymentOrderExecutionDto mapToDto(PaymentOrderExecution e) {
        return PaymentOrderExecutionDto.builder()
            .id(e.getId())
