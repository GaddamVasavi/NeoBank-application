...
/**
 * Transactional Service Implementation for PaymentWebhookEventLog
 */
@Service
@RequiredArgsConstructor
public class PaymentWebhookEventLogServiceImpl implements PaymentWebhookEventLogService {
    private final PaymentWebhookEventLogRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<PaymentWebhookEventLogDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<PaymentWebhookEventLogDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public PaymentWebhookEventLogDto create(PaymentWebhookEventLogDto dto) {
        ...
        PaymentWebhookEventLog entity = PaymentWebhookEventLog.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public PaymentWebhookEventLogDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        PaymentWebhookEventLog entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<PaymentWebhookEventLogDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private PaymentWebhookEventLogDto mapToDto(PaymentWebhookEventLog e) {
        return PaymentWebhookEventLogDto.builder()
            .id(e.getId())
