...
/**
 * Transactional Service Implementation for MerchantVirtualTerminal
 */
@Service
@RequiredArgsConstructor
public class MerchantVirtualTerminalServiceImpl implements MerchantVirtualTerminalService {
    private final MerchantVirtualTerminalRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<MerchantVirtualTerminalDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<MerchantVirtualTerminalDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public MerchantVirtualTerminalDto create(MerchantVirtualTerminalDto dto) {
        ...
        MerchantVirtualTerminal entity = MerchantVirtualTerminal.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public MerchantVirtualTerminalDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        MerchantVirtualTerminal entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<MerchantVirtualTerminalDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private MerchantVirtualTerminalDto mapToDto(MerchantVirtualTerminal e) {
        return MerchantVirtualTerminalDto.builder()
            .id(e.getId())
