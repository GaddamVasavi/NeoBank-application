...
/**
 * REST API Controller for PaymentIdempotencyKey
 */
@Tag(name = "Payments - PaymentIdempotencyKey")
@RestController
@RequestMapping("/api/v1/payments/modules/paymentidempotencykey")
...
public class PaymentIdempotencyKeyController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<PaymentIdempotencyKeyDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<PaymentIdempotencyKeyDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<PaymentIdempotencyKeyDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<PaymentIdempotencyKeyDto>> create(@Valid @RequestBody PaymentIdempotencyKeyDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<PaymentIdempotencyKeyDto>> getByRef(@PathVariable String ref) {
