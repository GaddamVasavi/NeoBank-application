...
/**
 * Transactional Service Implementation for PaymentIdempotencyKey
 */
@Service
@RequiredArgsConstructor
public class PaymentIdempotencyKeyServiceImpl implements PaymentIdempotencyKeyService {
    private final PaymentIdempotencyKeyRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<PaymentIdempotencyKeyDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<PaymentIdempotencyKeyDto> getByUser(UUID userId) {
    ...
    public PaymentIdempotencyKeyDto create(PaymentIdempotencyKeyDto dto) {
        ...
        PaymentIdempotencyKey entity = PaymentIdempotencyKey.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public PaymentIdempotencyKeyDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        PaymentIdempotencyKey entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<PaymentIdempotencyKeyDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private PaymentIdempotencyKeyDto mapToDto(PaymentIdempotencyKey e) {
        return PaymentIdempotencyKeyDto.builder()
            .id(e.getId())
