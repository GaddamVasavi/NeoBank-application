...
/**
 * Transactional Service Implementation for PaymentFeeAssessmentLog
 */
@Service
@RequiredArgsConstructor
public class PaymentFeeAssessmentLogServiceImpl implements PaymentFeeAssessmentLogService {
    private final PaymentFeeAssessmentLogRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<PaymentFeeAssessmentLogDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<PaymentFeeAssessmentLogDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public PaymentFeeAssessmentLogDto create(PaymentFeeAssessmentLogDto dto) {
        ...
        PaymentFeeAssessmentLog entity = PaymentFeeAssessmentLog.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public PaymentFeeAssessmentLogDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        PaymentFeeAssessmentLog entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<PaymentFeeAssessmentLogDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private PaymentFeeAssessmentLogDto mapToDto(PaymentFeeAssessmentLog e) {
        return PaymentFeeAssessmentLogDto.builder()
            .id(e.getId())
