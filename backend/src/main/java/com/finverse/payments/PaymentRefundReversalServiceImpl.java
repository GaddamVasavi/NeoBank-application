...
/**
 * Transactional Service Implementation for PaymentRefundReversal
 */
@Service
@RequiredArgsConstructor
public class PaymentRefundReversalServiceImpl implements PaymentRefundReversalService {
    private final PaymentRefundReversalRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<PaymentRefundReversalDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<PaymentRefundReversalDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public PaymentRefundReversalDto create(PaymentRefundReversalDto dto) {
        ...
        PaymentRefundReversal entity = PaymentRefundReversal.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public PaymentRefundReversalDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        PaymentRefundReversal entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<PaymentRefundReversalDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private PaymentRefundReversalDto mapToDto(PaymentRefundReversal e) {
        return PaymentRefundReversalDto.builder()
            .id(e.getId())
