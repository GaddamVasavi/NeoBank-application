...
/**
 * Transactional Service Implementation for PaymentRoutingRuleConfig
 */
@Service
@RequiredArgsConstructor
public class PaymentRoutingRuleConfigServiceImpl implements PaymentRoutingRuleConfigService {
    private final PaymentRoutingRuleConfigRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<PaymentRoutingRuleConfigDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<PaymentRoutingRuleConfigDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public PaymentRoutingRuleConfigDto create(PaymentRoutingRuleConfigDto dto) {
        ...
        PaymentRoutingRuleConfig entity = PaymentRoutingRuleConfig.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public PaymentRoutingRuleConfigDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        PaymentRoutingRuleConfig entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<PaymentRoutingRuleConfigDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private PaymentRoutingRuleConfigDto mapToDto(PaymentRoutingRuleConfig e) {
        return PaymentRoutingRuleConfigDto.builder()
            .id(e.getId())
