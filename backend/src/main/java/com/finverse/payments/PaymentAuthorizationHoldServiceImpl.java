...
/**
 * Transactional Service Implementation for PaymentAuthorizationHold
 */
@Service
@RequiredArgsConstructor
public class PaymentAuthorizationHoldServiceImpl implements PaymentAuthorizationHoldService {
    private final PaymentAuthorizationHoldRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<PaymentAuthorizationHoldDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<PaymentAuthorizationHoldDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public PaymentAuthorizationHoldDto create(PaymentAuthorizationHoldDto dto) {
        ...
        PaymentAuthorizationHold entity = PaymentAuthorizationHold.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public PaymentAuthorizationHoldDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        PaymentAuthorizationHold entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<PaymentAuthorizationHoldDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private PaymentAuthorizationHoldDto mapToDto(PaymentAuthorizationHold e) {
        return PaymentAuthorizationHoldDto.builder()
            .id(e.getId())
