...
/**
 * REST API Controller for PaymentRefundReversal
 */
@Tag(name = "Payments - PaymentRefundReversal")
@RestController
@RequestMapping("/api/v1/payments/modules/paymentrefundreversal")
@RequiredArgsConstructor
public class PaymentRefundReversalController {
    private final PaymentRefundReversalService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<PaymentRefundReversalDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<PaymentRefundReversalDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<PaymentRefundReversalDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<PaymentRefundReversalDto>> create(@Valid @RequestBody PaymentRefundReversalDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<PaymentRefundReversalDto>> getByRef(@PathVariable String ref) {
