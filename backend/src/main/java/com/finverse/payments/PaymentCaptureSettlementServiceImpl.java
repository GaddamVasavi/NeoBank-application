...
/**
 * Transactional Service Implementation for PaymentCaptureSettlement
 */
@Service
@RequiredArgsConstructor
public class PaymentCaptureSettlementServiceImpl implements PaymentCaptureSettlementService {
    private final PaymentCaptureSettlementRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<PaymentCaptureSettlementDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<PaymentCaptureSettlementDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public PaymentCaptureSettlementDto create(PaymentCaptureSettlementDto dto) {
        ...
        PaymentCaptureSettlement entity = PaymentCaptureSettlement.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public PaymentCaptureSettlementDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        PaymentCaptureSettlement entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<PaymentCaptureSettlementDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private PaymentCaptureSettlementDto mapToDto(PaymentCaptureSettlement e) {
        return PaymentCaptureSettlementDto.builder()
            .id(e.getId())
