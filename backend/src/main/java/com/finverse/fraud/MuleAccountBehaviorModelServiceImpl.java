...
/**
 * Transactional Service Implementation for MuleAccountBehaviorModel
 */
@Service
@RequiredArgsConstructor
public class MuleAccountBehaviorModelServiceImpl implements MuleAccountBehaviorModelService {
    private final MuleAccountBehaviorModelRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<MuleAccountBehaviorModelDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<MuleAccountBehaviorModelDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public MuleAccountBehaviorModelDto create(MuleAccountBehaviorModelDto dto) {
        ...
        MuleAccountBehaviorModel entity = MuleAccountBehaviorModel.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public MuleAccountBehaviorModelDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        MuleAccountBehaviorModel entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<MuleAccountBehaviorModelDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private MuleAccountBehaviorModelDto mapToDto(MuleAccountBehaviorModel e) {
        return MuleAccountBehaviorModelDto.builder()
            .id(e.getId())
