...
/**
 * REST API Controller for FraudRuleConfigMaster
 */
@Tag(name = "Fraud - FraudRuleConfigMaster")
@RestController
@RequestMapping("/api/v1/fraud/modules/fraudruleconfigmaster")
@RequiredArgsConstructor
public class FraudRuleConfigMasterController {
    private final FraudRuleConfigMasterService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<FraudRuleConfigMasterDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<FraudRuleConfigMasterDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<FraudRuleConfigMasterDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<FraudRuleConfigMasterDto>> create(@Valid @RequestBody FraudRuleConfigMasterDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<FraudRuleConfigMasterDto>> getByRef(@PathVariable String ref) {
