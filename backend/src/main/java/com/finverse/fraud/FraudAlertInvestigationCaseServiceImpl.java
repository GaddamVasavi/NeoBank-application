...
/**
 * Transactional Service Implementation for FraudAlertInvestigationCase
 */
@Service
@RequiredArgsConstructor
public class FraudAlertInvestigationCaseServiceImpl implements FraudAlertInvestigationCaseService {
    private final FraudAlertInvestigationCaseRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<FraudAlertInvestigationCaseDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<FraudAlertInvestigationCaseDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public FraudAlertInvestigationCaseDto create(FraudAlertInvestigationCaseDto dto) {
        ...
        FraudAlertInvestigationCase entity = FraudAlertInvestigationCase.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public FraudAlertInvestigationCaseDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        FraudAlertInvestigationCase entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<FraudAlertInvestigationCaseDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private FraudAlertInvestigationCaseDto mapToDto(FraudAlertInvestigationCase e) {
        return FraudAlertInvestigationCaseDto.builder()
            .id(e.getId())
