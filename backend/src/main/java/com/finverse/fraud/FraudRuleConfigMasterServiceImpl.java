...
/**
 * Transactional Service Implementation for FraudRuleConfigMaster
 */
@Service
@RequiredArgsConstructor
public class FraudRuleConfigMasterServiceImpl implements FraudRuleConfigMasterService {
    private final FraudRuleConfigMasterRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<FraudRuleConfigMasterDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<FraudRuleConfigMasterDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public FraudRuleConfigMasterDto create(FraudRuleConfigMasterDto dto) {
        ...
        FraudRuleConfigMaster entity = FraudRuleConfigMaster.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public FraudRuleConfigMasterDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        FraudRuleConfigMaster entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<FraudRuleConfigMasterDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private FraudRuleConfigMasterDto mapToDto(FraudRuleConfigMaster e) {
        return FraudRuleConfigMasterDto.builder()
            .id(e.getId())
