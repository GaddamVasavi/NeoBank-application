...
/**
 * Transactional Service Implementation for ChargebackFraudEvidence
 */
@Service
@RequiredArgsConstructor
public class ChargebackFraudEvidenceServiceImpl implements ChargebackFraudEvidenceService {
    private final ChargebackFraudEvidenceRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<ChargebackFraudEvidenceDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<ChargebackFraudEvidenceDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public ChargebackFraudEvidenceDto create(ChargebackFraudEvidenceDto dto) {
        ...
        ChargebackFraudEvidence entity = ChargebackFraudEvidence.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public ChargebackFraudEvidenceDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        ChargebackFraudEvidence entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<ChargebackFraudEvidenceDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private ChargebackFraudEvidenceDto mapToDto(ChargebackFraudEvidence e) {
        return ChargebackFraudEvidenceDto.builder()
            .id(e.getId())
