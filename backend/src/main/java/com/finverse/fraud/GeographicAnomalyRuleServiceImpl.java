...
/**
 * Transactional Service Implementation for GeographicAnomalyRule
 */
@Service
@RequiredArgsConstructor
public class GeographicAnomalyRuleServiceImpl implements GeographicAnomalyRuleService {
    private final GeographicAnomalyRuleRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<GeographicAnomalyRuleDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<GeographicAnomalyRuleDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public GeographicAnomalyRuleDto create(GeographicAnomalyRuleDto dto) {
        ...
        GeographicAnomalyRule entity = GeographicAnomalyRule.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public GeographicAnomalyRuleDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        GeographicAnomalyRule entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<GeographicAnomalyRuleDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private GeographicAnomalyRuleDto mapToDto(GeographicAnomalyRule e) {
        return GeographicAnomalyRuleDto.builder()
            .id(e.getId())
