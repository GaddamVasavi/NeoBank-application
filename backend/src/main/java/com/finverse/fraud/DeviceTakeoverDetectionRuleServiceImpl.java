...
/**
 * Transactional Service Implementation for DeviceTakeoverDetectionRule
 */
@Service
@RequiredArgsConstructor
public class DeviceTakeoverDetectionRuleServiceImpl implements DeviceTakeoverDetectionRuleService {
    private final DeviceTakeoverDetectionRuleRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<DeviceTakeoverDetectionRuleDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<DeviceTakeoverDetectionRuleDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public DeviceTakeoverDetectionRuleDto create(DeviceTakeoverDetectionRuleDto dto) {
        ...
        DeviceTakeoverDetectionRule entity = DeviceTakeoverDetectionRule.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public DeviceTakeoverDetectionRuleDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        DeviceTakeoverDetectionRule entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<DeviceTakeoverDetectionRuleDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private DeviceTakeoverDetectionRuleDto mapToDto(DeviceTakeoverDetectionRule e) {
        return DeviceTakeoverDetectionRuleDto.builder()
            .id(e.getId())
