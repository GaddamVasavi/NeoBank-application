...
/**
 * REST API Controller for GeographicAnomalyRule
 */
@Tag(name = "Fraud - GeographicAnomalyRule")
@RestController
@RequestMapping("/api/v1/fraud/modules/geographicanomalyrule")
@RequiredArgsConstructor
public class GeographicAnomalyRuleController {
    private final GeographicAnomalyRuleService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<GeographicAnomalyRuleDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<GeographicAnomalyRuleDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<GeographicAnomalyRuleDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<GeographicAnomalyRuleDto>> create(@Valid @RequestBody GeographicAnomalyRuleDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<GeographicAnomalyRuleDto>> getByRef(@PathVariable String ref) {
