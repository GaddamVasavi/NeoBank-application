...
/**
 * Transactional Service Implementation for CardTestingVelocityRule
 */
@Service
@RequiredArgsConstructor
public class CardTestingVelocityRuleServiceImpl implements CardTestingVelocityRuleService {
    private final CardTestingVelocityRuleRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CardTestingVelocityRuleDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CardTestingVelocityRuleDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CardTestingVelocityRuleDto create(CardTestingVelocityRuleDto dto) {
        ...
        CardTestingVelocityRule entity = CardTestingVelocityRule.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CardTestingVelocityRuleDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CardTestingVelocityRule entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CardTestingVelocityRuleDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CardTestingVelocityRuleDto mapToDto(CardTestingVelocityRule e) {
        return CardTestingVelocityRuleDto.builder()
            .id(e.getId())
