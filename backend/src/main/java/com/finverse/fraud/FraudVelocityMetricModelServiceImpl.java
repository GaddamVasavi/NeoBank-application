...
/**
 * Transactional Service Implementation for FraudVelocityMetricModel
 */
@Service
@RequiredArgsConstructor
public class FraudVelocityMetricModelServiceImpl implements FraudVelocityMetricModelService {
    private final FraudVelocityMetricModelRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<FraudVelocityMetricModelDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<FraudVelocityMetricModelDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public FraudVelocityMetricModelDto create(FraudVelocityMetricModelDto dto) {
        ...
        FraudVelocityMetricModel entity = FraudVelocityMetricModel.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public FraudVelocityMetricModelDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        FraudVelocityMetricModel entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<FraudVelocityMetricModelDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private FraudVelocityMetricModelDto mapToDto(FraudVelocityMetricModel e) {
        return FraudVelocityMetricModelDto.builder()
            .id(e.getId())
