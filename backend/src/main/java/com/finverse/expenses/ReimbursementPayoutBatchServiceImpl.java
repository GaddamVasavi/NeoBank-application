...
/**
 * Transactional Service Implementation for ReimbursementPayoutBatch
 */
@Service
@RequiredArgsConstructor
public class ReimbursementPayoutBatchServiceImpl implements ReimbursementPayoutBatchService {
    private final ReimbursementPayoutBatchRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<ReimbursementPayoutBatchDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<ReimbursementPayoutBatchDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public ReimbursementPayoutBatchDto create(ReimbursementPayoutBatchDto dto) {
        ...
        ReimbursementPayoutBatch entity = ReimbursementPayoutBatch.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public ReimbursementPayoutBatchDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        ReimbursementPayoutBatch entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<ReimbursementPayoutBatchDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private ReimbursementPayoutBatchDto mapToDto(ReimbursementPayoutBatch e) {
        return ReimbursementPayoutBatchDto.builder()
            .id(e.getId())
