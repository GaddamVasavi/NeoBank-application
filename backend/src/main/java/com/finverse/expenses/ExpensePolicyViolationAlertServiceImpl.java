...
/**
 * Transactional Service Implementation for ExpensePolicyViolationAlert
 */
@Service
@RequiredArgsConstructor
public class ExpensePolicyViolationAlertServiceImpl implements ExpensePolicyViolationAlertService {
    private final ExpensePolicyViolationAlertRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<ExpensePolicyViolationAlertDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<ExpensePolicyViolationAlertDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public ExpensePolicyViolationAlertDto create(ExpensePolicyViolationAlertDto dto) {
        ...
        ExpensePolicyViolationAlert entity = ExpensePolicyViolationAlert.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public ExpensePolicyViolationAlertDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        ExpensePolicyViolationAlert entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<ExpensePolicyViolationAlertDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private ExpensePolicyViolationAlertDto mapToDto(ExpensePolicyViolationAlert e) {
        return ExpensePolicyViolationAlertDto.builder()
            .id(e.getId())
