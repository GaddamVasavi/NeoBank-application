...
/**
 * Transactional Service Implementation for PerDiemAllowanceSchedule
 */
@Service
@RequiredArgsConstructor
public class PerDiemAllowanceScheduleServiceImpl implements PerDiemAllowanceScheduleService {
    private final PerDiemAllowanceScheduleRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<PerDiemAllowanceScheduleDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<PerDiemAllowanceScheduleDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public PerDiemAllowanceScheduleDto create(PerDiemAllowanceScheduleDto dto) {
        ...
        PerDiemAllowanceSchedule entity = PerDiemAllowanceSchedule.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public PerDiemAllowanceScheduleDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        PerDiemAllowanceSchedule entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<PerDiemAllowanceScheduleDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private PerDiemAllowanceScheduleDto mapToDto(PerDiemAllowanceSchedule e) {
        return PerDiemAllowanceScheduleDto.builder()
            .id(e.getId())
