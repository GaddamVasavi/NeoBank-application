...
/**
 * REST API Controller for ExpenseCategoryTaxMapping
 */
@Tag(name = "Expenses - ExpenseCategoryTaxMapping")
@RestController
@RequestMapping("/api/v1/expenses/modules/expensecategorytaxmapping")
@RequiredArgsConstructor
public class ExpenseCategoryTaxMappingController {
    private final ExpenseCategoryTaxMappingService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<ExpenseCategoryTaxMappingDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<ExpenseCategoryTaxMappingDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<ExpenseCategoryTaxMappingDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<ExpenseCategoryTaxMappingDto>> create(@Valid @RequestBody ExpenseCategoryTaxMappingDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<ExpenseCategoryTaxMappingDto>> getByRef(@PathVariable String ref) {
