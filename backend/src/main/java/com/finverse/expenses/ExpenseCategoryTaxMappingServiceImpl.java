...
/**
 * Transactional Service Implementation for ExpenseCategoryTaxMapping
 */
@Service
@RequiredArgsConstructor
public class ExpenseCategoryTaxMappingServiceImpl implements ExpenseCategoryTaxMappingService {
    private final ExpenseCategoryTaxMappingRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<ExpenseCategoryTaxMappingDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<ExpenseCategoryTaxMappingDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public ExpenseCategoryTaxMappingDto create(ExpenseCategoryTaxMappingDto dto) {
        ...
        ExpenseCategoryTaxMapping entity = ExpenseCategoryTaxMapping.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public ExpenseCategoryTaxMappingDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        ExpenseCategoryTaxMapping entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<ExpenseCategoryTaxMappingDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private ExpenseCategoryTaxMappingDto mapToDto(ExpenseCategoryTaxMapping e) {
        return ExpenseCategoryTaxMappingDto.builder()
            .id(e.getId())
