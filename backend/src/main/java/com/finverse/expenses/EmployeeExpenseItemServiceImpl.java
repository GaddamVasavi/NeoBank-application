...
/**
 * Transactional Service Implementation for EmployeeExpenseItem
 */
@Service
@RequiredArgsConstructor
public class EmployeeExpenseItemServiceImpl implements EmployeeExpenseItemService {
    private final EmployeeExpenseItemRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<EmployeeExpenseItemDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<EmployeeExpenseItemDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public EmployeeExpenseItemDto create(EmployeeExpenseItemDto dto) {
        ...
        EmployeeExpenseItem entity = EmployeeExpenseItem.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public EmployeeExpenseItemDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        EmployeeExpenseItem entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<EmployeeExpenseItemDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private EmployeeExpenseItemDto mapToDto(EmployeeExpenseItem e) {
        return EmployeeExpenseItemDto.builder()
            .id(e.getId())
