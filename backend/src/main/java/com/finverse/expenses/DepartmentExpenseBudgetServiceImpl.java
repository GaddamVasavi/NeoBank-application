...
/**
 * Transactional Service Implementation for DepartmentExpenseBudget
 */
@Service
@RequiredArgsConstructor
public class DepartmentExpenseBudgetServiceImpl implements DepartmentExpenseBudgetService {
    private final DepartmentExpenseBudgetRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<DepartmentExpenseBudgetDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<DepartmentExpenseBudgetDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public DepartmentExpenseBudgetDto create(DepartmentExpenseBudgetDto dto) {
        ...
        DepartmentExpenseBudget entity = DepartmentExpenseBudget.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public DepartmentExpenseBudgetDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        DepartmentExpenseBudget entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<DepartmentExpenseBudgetDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private DepartmentExpenseBudgetDto mapToDto(DepartmentExpenseBudget e) {
        return DepartmentExpenseBudgetDto.builder()
            .id(e.getId())
