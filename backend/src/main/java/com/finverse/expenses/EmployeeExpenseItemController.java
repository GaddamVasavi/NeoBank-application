...
/**
 * REST API Controller for EmployeeExpenseItem
 */
@Tag(name = "Expenses - EmployeeExpenseItem")
@RestController
@RequestMapping("/api/v1/expenses/modules/employeeexpenseitem")
@RequiredArgsConstructor
public class EmployeeExpenseItemController {
    private final EmployeeExpenseItemService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<EmployeeExpenseItemDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<EmployeeExpenseItemDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<EmployeeExpenseItemDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<EmployeeExpenseItemDto>> create(@Valid @RequestBody EmployeeExpenseItemDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<EmployeeExpenseItemDto>> getByRef(@PathVariable String ref) {
