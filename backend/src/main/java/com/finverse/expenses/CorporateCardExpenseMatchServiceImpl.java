...
/**
 * Transactional Service Implementation for CorporateCardExpenseMatch
 */
@Service
@RequiredArgsConstructor
public class CorporateCardExpenseMatchServiceImpl implements CorporateCardExpenseMatchService {
    private final CorporateCardExpenseMatchRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CorporateCardExpenseMatchDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CorporateCardExpenseMatchDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CorporateCardExpenseMatchDto create(CorporateCardExpenseMatchDto dto) {
        ...
        CorporateCardExpenseMatch entity = CorporateCardExpenseMatch.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CorporateCardExpenseMatchDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CorporateCardExpenseMatch entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CorporateCardExpenseMatchDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CorporateCardExpenseMatchDto mapToDto(CorporateCardExpenseMatch e) {
        return CorporateCardExpenseMatchDto.builder()
            .id(e.getId())
