...
/**
 * REST API Controller for MileageTravelLogEntry
 */
@Tag(name = "Expenses - MileageTravelLogEntry")
@RestController
@RequestMapping("/api/v1/expenses/modules/mileagetravellogentry")
@RequiredArgsConstructor
public class MileageTravelLogEntryController {
    private final MileageTravelLogEntryService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<MileageTravelLogEntryDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<MileageTravelLogEntryDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<MileageTravelLogEntryDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<MileageTravelLogEntryDto>> create(@Valid @RequestBody MileageTravelLogEntryDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<MileageTravelLogEntryDto>> getByRef(@PathVariable String ref) {
