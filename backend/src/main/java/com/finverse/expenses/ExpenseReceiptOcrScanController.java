...
/**
 * REST API Controller for ExpenseReceiptOcrScan
 */
@Tag(name = "Expenses - ExpenseReceiptOcrScan")
@RestController
@RequestMapping("/api/v1/expenses/modules/expensereceiptocrscan")
@RequiredArgsConstructor
public class ExpenseReceiptOcrScanController {
    private final ExpenseReceiptOcrScanService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<ExpenseReceiptOcrScanDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<ExpenseReceiptOcrScanDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<ExpenseReceiptOcrScanDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<ExpenseReceiptOcrScanDto>> create(@Valid @RequestBody ExpenseReceiptOcrScanDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<ExpenseReceiptOcrScanDto>> getByRef(@PathVariable String ref) {
