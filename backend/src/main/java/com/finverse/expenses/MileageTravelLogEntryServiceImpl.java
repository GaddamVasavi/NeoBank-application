...
/**
 * Transactional Service Implementation for MileageTravelLogEntry
 */
@Service
@RequiredArgsConstructor
public class MileageTravelLogEntryServiceImpl implements MileageTravelLogEntryService {
    private final MileageTravelLogEntryRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<MileageTravelLogEntryDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<MileageTravelLogEntryDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public MileageTravelLogEntryDto create(MileageTravelLogEntryDto dto) {
        ...
        MileageTravelLogEntry entity = MileageTravelLogEntry.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public MileageTravelLogEntryDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        MileageTravelLogEntry entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<MileageTravelLogEntryDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private MileageTravelLogEntryDto mapToDto(MileageTravelLogEntry e) {
        return MileageTravelLogEntryDto.builder()
            .id(e.getId())
