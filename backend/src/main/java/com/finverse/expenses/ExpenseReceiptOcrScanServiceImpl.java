...
/**
 * Transactional Service Implementation for ExpenseReceiptOcrScan
 */
@Service
@RequiredArgsConstructor
public class ExpenseReceiptOcrScanServiceImpl implements ExpenseReceiptOcrScanService {
    private final ExpenseReceiptOcrScanRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<ExpenseReceiptOcrScanDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<ExpenseReceiptOcrScanDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public ExpenseReceiptOcrScanDto create(ExpenseReceiptOcrScanDto dto) {
        ...
        ExpenseReceiptOcrScan entity = ExpenseReceiptOcrScan.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public ExpenseReceiptOcrScanDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        ExpenseReceiptOcrScan entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<ExpenseReceiptOcrScanDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private ExpenseReceiptOcrScanDto mapToDto(ExpenseReceiptOcrScan e) {
        return ExpenseReceiptOcrScanDto.builder()
            .id(e.getId())
