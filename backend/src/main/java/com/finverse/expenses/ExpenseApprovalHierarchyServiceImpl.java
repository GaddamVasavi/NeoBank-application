...
/**
 * Transactional Service Implementation for ExpenseApprovalHierarchy
 */
@Service
@RequiredArgsConstructor
public class ExpenseApprovalHierarchyServiceImpl implements ExpenseApprovalHierarchyService {
    private final ExpenseApprovalHierarchyRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<ExpenseApprovalHierarchyDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<ExpenseApprovalHierarchyDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public ExpenseApprovalHierarchyDto create(ExpenseApprovalHierarchyDto dto) {
        ...
        ExpenseApprovalHierarchy entity = ExpenseApprovalHierarchy.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public ExpenseApprovalHierarchyDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        ExpenseApprovalHierarchy entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<ExpenseApprovalHierarchyDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private ExpenseApprovalHierarchyDto mapToDto(ExpenseApprovalHierarchy e) {
        return ExpenseApprovalHierarchyDto.builder()
            .id(e.getId())
