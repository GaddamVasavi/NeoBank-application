...
/**
 * Transactional Service Implementation for TaxJurisdictionRate
 */
@Service
@RequiredArgsConstructor
public class TaxJurisdictionRateServiceImpl implements TaxJurisdictionRateService {
    private final TaxJurisdictionRateRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<TaxJurisdictionRateDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<TaxJurisdictionRateDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public TaxJurisdictionRateDto create(TaxJurisdictionRateDto dto) {
        ...
        TaxJurisdictionRate entity = TaxJurisdictionRate.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public TaxJurisdictionRateDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        TaxJurisdictionRate entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<TaxJurisdictionRateDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private TaxJurisdictionRateDto mapToDto(TaxJurisdictionRate e) {
        return TaxJurisdictionRateDto.builder()
            .id(e.getId())
