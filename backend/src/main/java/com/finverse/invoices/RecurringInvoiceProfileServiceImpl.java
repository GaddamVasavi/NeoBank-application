...
/**
 * Transactional Service Implementation for RecurringInvoiceProfile
 */
@Service
@RequiredArgsConstructor
public class RecurringInvoiceProfileServiceImpl implements RecurringInvoiceProfileService {
    private final RecurringInvoiceProfileRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<RecurringInvoiceProfileDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<RecurringInvoiceProfileDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public RecurringInvoiceProfileDto create(RecurringInvoiceProfileDto dto) {
        ...
        RecurringInvoiceProfile entity = RecurringInvoiceProfile.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public RecurringInvoiceProfileDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        RecurringInvoiceProfile entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<RecurringInvoiceProfileDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private RecurringInvoiceProfileDto mapToDto(RecurringInvoiceProfile e) {
        return RecurringInvoiceProfileDto.builder()
            .id(e.getId())
