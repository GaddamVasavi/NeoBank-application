...
/**
 * Transactional Service Implementation for InvoicePaymentAllocation
 */
@Service
@RequiredArgsConstructor
public class InvoicePaymentAllocationServiceImpl implements InvoicePaymentAllocationService {
    private final InvoicePaymentAllocationRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<InvoicePaymentAllocationDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<InvoicePaymentAllocationDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public InvoicePaymentAllocationDto create(InvoicePaymentAllocationDto dto) {
        ...
        InvoicePaymentAllocation entity = InvoicePaymentAllocation.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public InvoicePaymentAllocationDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        InvoicePaymentAllocation entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<InvoicePaymentAllocationDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private InvoicePaymentAllocationDto mapToDto(InvoicePaymentAllocation e) {
        return InvoicePaymentAllocationDto.builder()
            .id(e.getId())
