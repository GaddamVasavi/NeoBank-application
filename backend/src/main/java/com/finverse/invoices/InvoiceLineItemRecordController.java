...
/**
 * REST API Controller for InvoiceLineItemRecord
 */
@Tag(name = "Invoices - InvoiceLineItemRecord")
@RestController
@RequestMapping("/api/v1/invoices/modules/invoicelineitemrecord")
@RequiredArgsConstructor
public class InvoiceLineItemRecordController {
    private final InvoiceLineItemRecordService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<InvoiceLineItemRecordDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<InvoiceLineItemRecordDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<InvoiceLineItemRecordDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<InvoiceLineItemRecordDto>> create(@Valid @RequestBody InvoiceLineItemRecordDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<InvoiceLineItemRecordDto>> getByRef(@PathVariable String ref) {
