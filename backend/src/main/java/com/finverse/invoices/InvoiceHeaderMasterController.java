...
/**
 * REST API Controller for InvoiceHeaderMaster
 */
@Tag(name = "Invoices - InvoiceHeaderMaster")
@RestController
@RequestMapping("/api/v1/invoices/modules/invoiceheadermaster")
@RequiredArgsConstructor
public class InvoiceHeaderMasterController {
    private final InvoiceHeaderMasterService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<InvoiceHeaderMasterDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<InvoiceHeaderMasterDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<InvoiceHeaderMasterDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<InvoiceHeaderMasterDto>> create(@Valid @RequestBody InvoiceHeaderMasterDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<InvoiceHeaderMasterDto>> getByRef(@PathVariable String ref) {
