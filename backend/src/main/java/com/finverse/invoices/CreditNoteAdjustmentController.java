...
/**
 * REST API Controller for CreditNoteAdjustment
 */
@Tag(name = "Invoices - CreditNoteAdjustment")
@RestController
@RequestMapping("/api/v1/invoices/modules/creditnoteadjustment")
@RequiredArgsConstructor
public class CreditNoteAdjustmentController {
    private final CreditNoteAdjustmentService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<CreditNoteAdjustmentDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<CreditNoteAdjustmentDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<CreditNoteAdjustmentDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<CreditNoteAdjustmentDto>> create(@Valid @RequestBody CreditNoteAdjustmentDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<CreditNoteAdjustmentDto>> getByRef(@PathVariable String ref) {
