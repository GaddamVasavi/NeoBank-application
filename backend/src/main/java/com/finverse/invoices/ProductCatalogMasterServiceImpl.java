...
/**
 * Transactional Service Implementation for ProductCatalogMaster
 */
@Service
@RequiredArgsConstructor
public class ProductCatalogMasterServiceImpl implements ProductCatalogMasterService {
    private final ProductCatalogMasterRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<ProductCatalogMasterDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<ProductCatalogMasterDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public ProductCatalogMasterDto create(ProductCatalogMasterDto dto) {
        ...
        ProductCatalogMaster entity = ProductCatalogMaster.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public ProductCatalogMasterDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        ProductCatalogMaster entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<ProductCatalogMasterDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private ProductCatalogMasterDto mapToDto(ProductCatalogMaster e) {
        return ProductCatalogMasterDto.builder()
            .id(e.getId())
