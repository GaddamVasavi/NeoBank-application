...
/**
 * Transactional Service Implementation for CreditNoteAdjustment
 */
@Service
@RequiredArgsConstructor
public class CreditNoteAdjustmentServiceImpl implements CreditNoteAdjustmentService {
    private final CreditNoteAdjustmentRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CreditNoteAdjustmentDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CreditNoteAdjustmentDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CreditNoteAdjustmentDto create(CreditNoteAdjustmentDto dto) {
        ...
        CreditNoteAdjustment entity = CreditNoteAdjustment.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CreditNoteAdjustmentDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CreditNoteAdjustment entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CreditNoteAdjustmentDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CreditNoteAdjustmentDto mapToDto(CreditNoteAdjustment e) {
        return CreditNoteAdjustmentDto.builder()
            .id(e.getId())
