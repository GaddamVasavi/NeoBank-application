...
/**
 * Transactional Service Implementation for DunningScheduleWorkflow
 */
@Service
@RequiredArgsConstructor
public class DunningScheduleWorkflowServiceImpl implements DunningScheduleWorkflowService {
    private final DunningScheduleWorkflowRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<DunningScheduleWorkflowDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<DunningScheduleWorkflowDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public DunningScheduleWorkflowDto create(DunningScheduleWorkflowDto dto) {
        ...
        DunningScheduleWorkflow entity = DunningScheduleWorkflow.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public DunningScheduleWorkflowDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        DunningScheduleWorkflow entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<DunningScheduleWorkflowDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private DunningScheduleWorkflowDto mapToDto(DunningScheduleWorkflow e) {
        return DunningScheduleWorkflowDto.builder()
            .id(e.getId())
