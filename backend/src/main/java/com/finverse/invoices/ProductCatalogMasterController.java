...
/**
 * REST API Controller for ProductCatalogMaster
 */
@Tag(name = "Invoices - ProductCatalogMaster")
@RestController
@RequestMapping("/api/v1/invoices/modules/productcatalogmaster")
@RequiredArgsConstructor
public class ProductCatalogMasterController {
    private final ProductCatalogMasterService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<ProductCatalogMasterDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<ProductCatalogMasterDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<ProductCatalogMasterDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<ProductCatalogMasterDto>> create(@Valid @RequestBody ProductCatalogMasterDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<ProductCatalogMasterDto>> getByRef(@PathVariable String ref) {
