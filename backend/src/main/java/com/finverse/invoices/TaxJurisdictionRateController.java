...
/**
 * REST API Controller for TaxJurisdictionRate
 */
@Tag(name = "Invoices - TaxJurisdictionRate")
@RestController
@RequestMapping("/api/v1/invoices/modules/taxjurisdictionrate")
@RequiredArgsConstructor
public class TaxJurisdictionRateController {
    private final TaxJurisdictionRateService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<TaxJurisdictionRateDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<TaxJurisdictionRateDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<TaxJurisdictionRateDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<TaxJurisdictionRateDto>> create(@Valid @RequestBody TaxJurisdictionRateDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<TaxJurisdictionRateDto>> getByRef(@PathVariable String ref) {
