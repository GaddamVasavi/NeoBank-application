...
/**
 * Transactional Service Implementation for CustomerBillingProfile
 */
@Service
@RequiredArgsConstructor
public class CustomerBillingProfileServiceImpl implements CustomerBillingProfileService {
    private final CustomerBillingProfileRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CustomerBillingProfileDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CustomerBillingProfileDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CustomerBillingProfileDto create(CustomerBillingProfileDto dto) {
        ...
        CustomerBillingProfile entity = CustomerBillingProfile.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CustomerBillingProfileDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CustomerBillingProfile entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CustomerBillingProfileDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CustomerBillingProfileDto mapToDto(CustomerBillingProfile e) {
        return CustomerBillingProfileDto.builder()
            .id(e.getId())
