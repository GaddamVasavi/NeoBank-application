...
/**
 * Transactional Service Implementation for InvoiceHeaderMaster
 */
@Service
@RequiredArgsConstructor
public class InvoiceHeaderMasterServiceImpl implements InvoiceHeaderMasterService {
    private final InvoiceHeaderMasterRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<InvoiceHeaderMasterDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<InvoiceHeaderMasterDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public InvoiceHeaderMasterDto create(InvoiceHeaderMasterDto dto) {
        ...
        InvoiceHeaderMaster entity = InvoiceHeaderMaster.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public InvoiceHeaderMasterDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        InvoiceHeaderMaster entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<InvoiceHeaderMasterDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private InvoiceHeaderMasterDto mapToDto(InvoiceHeaderMaster e) {
        return InvoiceHeaderMasterDto.builder()
            .id(e.getId())
