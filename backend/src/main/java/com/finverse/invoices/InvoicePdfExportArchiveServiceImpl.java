...
/**
 * Transactional Service Implementation for InvoicePdfExportArchive
 */
@Service
@RequiredArgsConstructor
public class InvoicePdfExportArchiveServiceImpl implements InvoicePdfExportArchiveService {
    private final InvoicePdfExportArchiveRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<InvoicePdfExportArchiveDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<InvoicePdfExportArchiveDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public InvoicePdfExportArchiveDto create(InvoicePdfExportArchiveDto dto) {
        ...
        InvoicePdfExportArchive entity = InvoicePdfExportArchive.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public InvoicePdfExportArchiveDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        InvoicePdfExportArchive entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<InvoicePdfExportArchiveDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private InvoicePdfExportArchiveDto mapToDto(InvoicePdfExportArchive e) {
        return InvoicePdfExportArchiveDto.builder()
            .id(e.getId())
