...
/**
 * Transactional Service Implementation for InvoiceLineItemRecord
 */
@Service
@RequiredArgsConstructor
public class InvoiceLineItemRecordServiceImpl implements InvoiceLineItemRecordService {
    private final InvoiceLineItemRecordRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<InvoiceLineItemRecordDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<InvoiceLineItemRecordDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public InvoiceLineItemRecordDto create(InvoiceLineItemRecordDto dto) {
        ...
        InvoiceLineItemRecord entity = InvoiceLineItemRecord.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public InvoiceLineItemRecordDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        InvoiceLineItemRecord entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<InvoiceLineItemRecordDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private InvoiceLineItemRecordDto mapToDto(InvoiceLineItemRecord e) {
        return InvoiceLineItemRecordDto.builder()
            .id(e.getId())
