...
/**
 * Transactional Service Implementation for SubAccountHierarchyNode
 */
@Service
@RequiredArgsConstructor
public class SubAccountHierarchyNodeServiceImpl implements SubAccountHierarchyNodeService {
    private final SubAccountHierarchyNodeRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<SubAccountHierarchyNodeDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<SubAccountHierarchyNodeDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public SubAccountHierarchyNodeDto create(SubAccountHierarchyNodeDto dto) {
        ...
        SubAccountHierarchyNode entity = SubAccountHierarchyNode.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public SubAccountHierarchyNodeDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        SubAccountHierarchyNode entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<SubAccountHierarchyNodeDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private SubAccountHierarchyNodeDto mapToDto(SubAccountHierarchyNode e) {
        return SubAccountHierarchyNodeDto.builder()
            .id(e.getId())
