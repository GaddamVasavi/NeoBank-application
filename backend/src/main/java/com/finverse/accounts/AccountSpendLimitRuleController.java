...
/**
 * REST API Controller for AccountSpendLimitRule
 */
@Tag(name = "Accounts - AccountSpendLimitRule")
@RestController
@RequestMapping("/api/v1/accounts/modules/accountspendlimitrule")
@RequiredArgsConstructor
public class AccountSpendLimitRuleController {
    private final AccountSpendLimitRuleService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<AccountSpendLimitRuleDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<AccountSpendLimitRuleDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<AccountSpendLimitRuleDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<AccountSpendLimitRuleDto>> create(@Valid @RequestBody AccountSpendLimitRuleDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<AccountSpendLimitRuleDto>> getByRef(@PathVariable String ref) {
