...
/**
 * Transactional Service Implementation for AccountInterestAccrualLog
 */
@Service
@RequiredArgsConstructor
public class AccountInterestAccrualLogServiceImpl implements AccountInterestAccrualLogService {
    private final AccountInterestAccrualLogRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<AccountInterestAccrualLogDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<AccountInterestAccrualLogDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public AccountInterestAccrualLogDto create(AccountInterestAccrualLogDto dto) {
        ...
        AccountInterestAccrualLog entity = AccountInterestAccrualLog.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public AccountInterestAccrualLogDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        AccountInterestAccrualLog entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<AccountInterestAccrualLogDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private AccountInterestAccrualLogDto mapToDto(AccountInterestAccrualLog e) {
        return AccountInterestAccrualLogDto.builder()
            .id(e.getId())
