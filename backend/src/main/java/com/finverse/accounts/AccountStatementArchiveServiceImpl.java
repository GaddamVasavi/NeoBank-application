...
/**
 * Transactional Service Implementation for AccountStatementArchive
 */
@Service
@RequiredArgsConstructor
public class AccountStatementArchiveServiceImpl implements AccountStatementArchiveService {
    private final AccountStatementArchiveRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<AccountStatementArchiveDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<AccountStatementArchiveDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public AccountStatementArchiveDto create(AccountStatementArchiveDto dto) {
        ...
        AccountStatementArchive entity = AccountStatementArchive.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public AccountStatementArchiveDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        AccountStatementArchive entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<AccountStatementArchiveDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private AccountStatementArchiveDto mapToDto(AccountStatementArchive e) {
        return AccountStatementArchiveDto.builder()
            .id(e.getId())
