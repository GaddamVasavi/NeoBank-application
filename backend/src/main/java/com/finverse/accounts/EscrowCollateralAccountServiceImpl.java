...
/**
 * Transactional Service Implementation for EscrowCollateralAccount
 */
@Service
@RequiredArgsConstructor
public class EscrowCollateralAccountServiceImpl implements EscrowCollateralAccountService {
    private final EscrowCollateralAccountRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<EscrowCollateralAccountDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<EscrowCollateralAccountDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public EscrowCollateralAccountDto create(EscrowCollateralAccountDto dto) {
        ...
        EscrowCollateralAccount entity = EscrowCollateralAccount.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public EscrowCollateralAccountDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        EscrowCollateralAccount entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<EscrowCollateralAccountDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private EscrowCollateralAccountDto mapToDto(EscrowCollateralAccount e) {
        return EscrowCollateralAccountDto.builder()
            .id(e.getId())
