...
/**
 * Transactional Service Implementation for AccountSpendLimitRule
 */
@Service
@RequiredArgsConstructor
public class AccountSpendLimitRuleServiceImpl implements AccountSpendLimitRuleService {
    private final AccountSpendLimitRuleRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<AccountSpendLimitRuleDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<AccountSpendLimitRuleDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public AccountSpendLimitRuleDto create(AccountSpendLimitRuleDto dto) {
        ...
        AccountSpendLimitRule entity = AccountSpendLimitRule.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public AccountSpendLimitRuleDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        AccountSpendLimitRule entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<AccountSpendLimitRuleDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private AccountSpendLimitRuleDto mapToDto(AccountSpendLimitRule e) {
        return AccountSpendLimitRuleDto.builder()
            .id(e.getId())
