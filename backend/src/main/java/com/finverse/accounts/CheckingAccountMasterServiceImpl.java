...
/**
 * Transactional Service Implementation for CheckingAccountMaster
 */
@Service
@RequiredArgsConstructor
public class CheckingAccountMasterServiceImpl implements CheckingAccountMasterService {
    private final CheckingAccountMasterRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CheckingAccountMasterDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CheckingAccountMasterDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CheckingAccountMasterDto create(CheckingAccountMasterDto dto) {
        ...
        CheckingAccountMaster entity = CheckingAccountMaster.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CheckingAccountMasterDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CheckingAccountMaster entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CheckingAccountMasterDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CheckingAccountMasterDto mapToDto(CheckingAccountMaster e) {
        return CheckingAccountMasterDto.builder()
            .id(e.getId())
