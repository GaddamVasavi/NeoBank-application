...
/**
 * Transactional Service Implementation for CorporateTreasuryWallet
 */
@Service
@RequiredArgsConstructor
public class CorporateTreasuryWalletServiceImpl implements CorporateTreasuryWalletService {
    private final CorporateTreasuryWalletRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CorporateTreasuryWalletDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CorporateTreasuryWalletDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CorporateTreasuryWalletDto create(CorporateTreasuryWalletDto dto) {
        ...
        CorporateTreasuryWallet entity = CorporateTreasuryWallet.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CorporateTreasuryWalletDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CorporateTreasuryWallet entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CorporateTreasuryWalletDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CorporateTreasuryWalletDto mapToDto(CorporateTreasuryWallet e) {
        return CorporateTreasuryWalletDto.builder()
            .id(e.getId())
