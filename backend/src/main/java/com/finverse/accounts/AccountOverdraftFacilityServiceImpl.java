...
/**
 * Transactional Service Implementation for AccountOverdraftFacility
 */
@Service
@RequiredArgsConstructor
public class AccountOverdraftFacilityServiceImpl implements AccountOverdraftFacilityService {
    private final AccountOverdraftFacilityRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<AccountOverdraftFacilityDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<AccountOverdraftFacilityDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public AccountOverdraftFacilityDto create(AccountOverdraftFacilityDto dto) {
        ...
        AccountOverdraftFacility entity = AccountOverdraftFacility.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public AccountOverdraftFacilityDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        AccountOverdraftFacility entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<AccountOverdraftFacilityDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private AccountOverdraftFacilityDto mapToDto(AccountOverdraftFacility e) {
        return AccountOverdraftFacilityDto.builder()
            .id(e.getId())
