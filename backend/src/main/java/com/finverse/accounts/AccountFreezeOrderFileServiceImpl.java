...
/**
 * Transactional Service Implementation for AccountFreezeOrderFile
 */
@Service
@RequiredArgsConstructor
public class AccountFreezeOrderFileServiceImpl implements AccountFreezeOrderFileService {
    private final AccountFreezeOrderFileRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<AccountFreezeOrderFileDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<AccountFreezeOrderFileDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public AccountFreezeOrderFileDto create(AccountFreezeOrderFileDto dto) {
        ...
        AccountFreezeOrderFile entity = AccountFreezeOrderFile.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public AccountFreezeOrderFileDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        AccountFreezeOrderFile entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<AccountFreezeOrderFileDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private AccountFreezeOrderFileDto mapToDto(AccountFreezeOrderFile e) {
        return AccountFreezeOrderFileDto.builder()
            .id(e.getId())
