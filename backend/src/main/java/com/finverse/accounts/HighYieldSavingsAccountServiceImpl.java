...
/**
 * Transactional Service Implementation for HighYieldSavingsAccount
 */
@Service
@RequiredArgsConstructor
public class HighYieldSavingsAccountServiceImpl implements HighYieldSavingsAccountService {
    private final HighYieldSavingsAccountRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<HighYieldSavingsAccountDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<HighYieldSavingsAccountDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public HighYieldSavingsAccountDto create(HighYieldSavingsAccountDto dto) {
        ...
        HighYieldSavingsAccount entity = HighYieldSavingsAccount.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public HighYieldSavingsAccountDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        HighYieldSavingsAccount entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<HighYieldSavingsAccountDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private HighYieldSavingsAccountDto mapToDto(HighYieldSavingsAccount e) {
        return HighYieldSavingsAccountDto.builder()
            .id(e.getId())
