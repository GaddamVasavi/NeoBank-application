...
/**
 * REST API Controller for CheckingAccountMaster
 */
@Tag(name = "Accounts - CheckingAccountMaster")
@RestController
@RequestMapping("/api/v1/accounts/modules/checkingaccountmaster")
@RequiredArgsConstructor
public class CheckingAccountMasterController {
    private final CheckingAccountMasterService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<CheckingAccountMasterDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<CheckingAccountMasterDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<CheckingAccountMasterDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<CheckingAccountMasterDto>> create(@Valid @RequestBody CheckingAccountMasterDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<CheckingAccountMasterDto>> getByRef(@PathVariable String ref) {
