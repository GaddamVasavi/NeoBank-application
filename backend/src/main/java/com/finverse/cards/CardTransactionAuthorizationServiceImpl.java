...
/**
 * Transactional Service Implementation for CardTransactionAuthorization
 */
@Service
@RequiredArgsConstructor
public class CardTransactionAuthorizationServiceImpl implements CardTransactionAuthorizationService {
    private final CardTransactionAuthorizationRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CardTransactionAuthorizationDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CardTransactionAuthorizationDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CardTransactionAuthorizationDto create(CardTransactionAuthorizationDto dto) {
        ...
        CardTransactionAuthorization entity = CardTransactionAuthorization.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CardTransactionAuthorizationDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CardTransactionAuthorization entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CardTransactionAuthorizationDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CardTransactionAuthorizationDto mapToDto(CardTransactionAuthorization e) {
        return CardTransactionAuthorizationDto.builder()
            .id(e.getId())
