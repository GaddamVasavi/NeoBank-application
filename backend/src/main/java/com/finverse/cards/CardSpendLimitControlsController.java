...
/**
 * REST API Controller for CardSpendLimitControls
 */
@Tag(name = "Cards - CardSpendLimitControls")
@RestController
@RequestMapping("/api/v1/cards/modules/cardspendlimitcontrols")
@RequiredArgsConstructor
public class CardSpendLimitControlsController {
    private final CardSpendLimitControlsService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<CardSpendLimitControlsDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<CardSpendLimitControlsDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<CardSpendLimitControlsDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<CardSpendLimitControlsDto>> create(@Valid @RequestBody CardSpendLimitControlsDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<CardSpendLimitControlsDto>> getByRef(@PathVariable String ref) {
