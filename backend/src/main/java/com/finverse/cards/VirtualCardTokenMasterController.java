...
/**
 * REST API Controller for VirtualCardTokenMaster
 */
@Tag(name = "Cards - VirtualCardTokenMaster")
@RestController
@RequestMapping("/api/v1/cards/modules/virtualcardtokenmaster")
@RequiredArgsConstructor
public class VirtualCardTokenMasterController {
    private final VirtualCardTokenMasterService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<VirtualCardTokenMasterDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<VirtualCardTokenMasterDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<VirtualCardTokenMasterDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<VirtualCardTokenMasterDto>> create(@Valid @RequestBody VirtualCardTokenMasterDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<VirtualCardTokenMasterDto>> getByRef(@PathVariable String ref) {
