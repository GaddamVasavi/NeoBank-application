...
/**
 * Transactional Service Implementation for CardMonthlyStatementFile
 */
@Service
@RequiredArgsConstructor
public class CardMonthlyStatementFileServiceImpl implements CardMonthlyStatementFileService {
    private final CardMonthlyStatementFileRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CardMonthlyStatementFileDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CardMonthlyStatementFileDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CardMonthlyStatementFileDto create(CardMonthlyStatementFileDto dto) {
        ...
        CardMonthlyStatementFile entity = CardMonthlyStatementFile.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CardMonthlyStatementFileDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CardMonthlyStatementFile entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CardMonthlyStatementFileDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CardMonthlyStatementFileDto mapToDto(CardMonthlyStatementFile e) {
        return CardMonthlyStatementFileDto.builder()
            .id(e.getId())
