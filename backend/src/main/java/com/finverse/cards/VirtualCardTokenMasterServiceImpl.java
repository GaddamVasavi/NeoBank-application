...
/**
 * Transactional Service Implementation for VirtualCardTokenMaster
 */
@Service
@RequiredArgsConstructor
public class VirtualCardTokenMasterServiceImpl implements VirtualCardTokenMasterService {
    private final VirtualCardTokenMasterRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<VirtualCardTokenMasterDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<VirtualCardTokenMasterDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public VirtualCardTokenMasterDto create(VirtualCardTokenMasterDto dto) {
        ...
        VirtualCardTokenMaster entity = VirtualCardTokenMaster.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public VirtualCardTokenMasterDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        VirtualCardTokenMaster entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<VirtualCardTokenMasterDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private VirtualCardTokenMasterDto mapToDto(VirtualCardTokenMaster e) {
        return VirtualCardTokenMasterDto.builder()
            .id(e.getId())
