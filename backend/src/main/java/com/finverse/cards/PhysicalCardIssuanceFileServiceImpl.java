...
/**
 * Transactional Service Implementation for PhysicalCardIssuanceFile
 */
@Service
@RequiredArgsConstructor
public class PhysicalCardIssuanceFileServiceImpl implements PhysicalCardIssuanceFileService {
    private final PhysicalCardIssuanceFileRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<PhysicalCardIssuanceFileDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<PhysicalCardIssuanceFileDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public PhysicalCardIssuanceFileDto create(PhysicalCardIssuanceFileDto dto) {
        ...
        PhysicalCardIssuanceFile entity = PhysicalCardIssuanceFile.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public PhysicalCardIssuanceFileDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        PhysicalCardIssuanceFile entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<PhysicalCardIssuanceFileDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private PhysicalCardIssuanceFileDto mapToDto(PhysicalCardIssuanceFile e) {
        return PhysicalCardIssuanceFileDto.builder()
            .id(e.getId())
