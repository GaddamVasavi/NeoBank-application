...
/**
 * Transactional Service Implementation for CardPinSecurityToken
 */
@Service
@RequiredArgsConstructor
public class CardPinSecurityTokenServiceImpl implements CardPinSecurityTokenService {
    private final CardPinSecurityTokenRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CardPinSecurityTokenDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CardPinSecurityTokenDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CardPinSecurityTokenDto create(CardPinSecurityTokenDto dto) {
        ...
        CardPinSecurityToken entity = CardPinSecurityToken.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CardPinSecurityTokenDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CardPinSecurityToken entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CardPinSecurityTokenDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CardPinSecurityTokenDto mapToDto(CardPinSecurityToken e) {
        return CardPinSecurityTokenDto.builder()
            .id(e.getId())
