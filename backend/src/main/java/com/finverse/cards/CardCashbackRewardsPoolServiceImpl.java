...
/**
 * Transactional Service Implementation for CardCashbackRewardsPool
 */
@Service
@RequiredArgsConstructor
public class CardCashbackRewardsPoolServiceImpl implements CardCashbackRewardsPoolService {
    private final CardCashbackRewardsPoolRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CardCashbackRewardsPoolDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CardCashbackRewardsPoolDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CardCashbackRewardsPoolDto create(CardCashbackRewardsPoolDto dto) {
        ...
        CardCashbackRewardsPool entity = CardCashbackRewardsPool.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CardCashbackRewardsPoolDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CardCashbackRewardsPool entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CardCashbackRewardsPoolDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CardCashbackRewardsPoolDto mapToDto(CardCashbackRewardsPool e) {
        return CardCashbackRewardsPoolDto.builder()
            .id(e.getId())
