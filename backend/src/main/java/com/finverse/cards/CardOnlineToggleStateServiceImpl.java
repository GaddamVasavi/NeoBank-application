...
/**
 * Transactional Service Implementation for CardOnlineToggleState
 */
@Service
@RequiredArgsConstructor
public class CardOnlineToggleStateServiceImpl implements CardOnlineToggleStateService {
    private final CardOnlineToggleStateRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CardOnlineToggleStateDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CardOnlineToggleStateDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CardOnlineToggleStateDto create(CardOnlineToggleStateDto dto) {
        ...
        CardOnlineToggleState entity = CardOnlineToggleState.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CardOnlineToggleStateDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CardOnlineToggleState entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CardOnlineToggleStateDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CardOnlineToggleStateDto mapToDto(CardOnlineToggleState e) {
        return CardOnlineToggleStateDto.builder()
            .id(e.getId())
