...
/**
 * REST API Controller for CardOnlineToggleState
 */
@Tag(name = "Cards - CardOnlineToggleState")
@RestController
@RequestMapping("/api/v1/cards/modules/cardonlinetogglestate")
@RequiredArgsConstructor
public class CardOnlineToggleStateController {
    private final CardOnlineToggleStateService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<CardOnlineToggleStateDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<CardOnlineToggleStateDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<CardOnlineToggleStateDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<CardOnlineToggleStateDto>> create(@Valid @RequestBody CardOnlineToggleStateDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<CardOnlineToggleStateDto>> getByRef(@PathVariable String ref) {
