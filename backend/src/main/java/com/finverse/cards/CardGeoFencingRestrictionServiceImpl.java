...
/**
 * Transactional Service Implementation for CardGeoFencingRestriction
 */
@Service
@RequiredArgsConstructor
public class CardGeoFencingRestrictionServiceImpl implements CardGeoFencingRestrictionService {
    private final CardGeoFencingRestrictionRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CardGeoFencingRestrictionDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CardGeoFencingRestrictionDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CardGeoFencingRestrictionDto create(CardGeoFencingRestrictionDto dto) {
        ...
        CardGeoFencingRestriction entity = CardGeoFencingRestriction.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CardGeoFencingRestrictionDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CardGeoFencingRestriction entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CardGeoFencingRestrictionDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CardGeoFencingRestrictionDto mapToDto(CardGeoFencingRestriction e) {
        return CardGeoFencingRestrictionDto.builder()
            .id(e.getId())
