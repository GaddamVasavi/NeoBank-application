...
/**
 * Transactional Service Implementation for CardSpendLimitControls
 */
@Service
@RequiredArgsConstructor
public class CardSpendLimitControlsServiceImpl implements CardSpendLimitControlsService {
    private final CardSpendLimitControlsRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<CardSpendLimitControlsDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<CardSpendLimitControlsDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public CardSpendLimitControlsDto create(CardSpendLimitControlsDto dto) {
        ...
        CardSpendLimitControls entity = CardSpendLimitControls.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public CardSpendLimitControlsDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        CardSpendLimitControls entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CardSpendLimitControlsDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private CardSpendLimitControlsDto mapToDto(CardSpendLimitControls e) {
        return CardSpendLimitControlsDto.builder()
            .id(e.getId())
