...
/**
 * Transactional Service Implementation for RegulatoryCapitalAdequacyReport
 */
@Service
@RequiredArgsConstructor
public class RegulatoryCapitalAdequacyReportServiceImpl implements RegulatoryCapitalAdequacyReportService {
    private final RegulatoryCapitalAdequacyReportRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<RegulatoryCapitalAdequacyReportDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<RegulatoryCapitalAdequacyReportDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public RegulatoryCapitalAdequacyReportDto create(RegulatoryCapitalAdequacyReportDto dto) {
        ...
        RegulatoryCapitalAdequacyReport entity = RegulatoryCapitalAdequacyReport.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public RegulatoryCapitalAdequacyReportDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        RegulatoryCapitalAdequacyReport entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<RegulatoryCapitalAdequacyReportDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private RegulatoryCapitalAdequacyReportDto mapToDto(RegulatoryCapitalAdequacyReport e) {
        return RegulatoryCapitalAdequacyReportDto.builder()
            .id(e.getId())
