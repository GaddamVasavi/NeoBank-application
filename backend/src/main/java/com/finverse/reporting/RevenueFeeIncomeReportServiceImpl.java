...
/**
 * Transactional Service Implementation for RevenueFeeIncomeReport
 */
@Service
@RequiredArgsConstructor
public class RevenueFeeIncomeReportServiceImpl implements RevenueFeeIncomeReportService {
    private final RevenueFeeIncomeReportRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<RevenueFeeIncomeReportDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<RevenueFeeIncomeReportDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public RevenueFeeIncomeReportDto create(RevenueFeeIncomeReportDto dto) {
        ...
        RevenueFeeIncomeReport entity = RevenueFeeIncomeReport.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public RevenueFeeIncomeReportDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        RevenueFeeIncomeReport entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<RevenueFeeIncomeReportDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private RevenueFeeIncomeReportDto mapToDto(RevenueFeeIncomeReport e) {
        return RevenueFeeIncomeReportDto.builder()
            .id(e.getId())
