...
/**
 * Transactional Service Implementation for DailyFinancialSummaryReport
 */
@Service
@RequiredArgsConstructor
public class DailyFinancialSummaryReportServiceImpl implements DailyFinancialSummaryReportService {
    private final DailyFinancialSummaryReportRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<DailyFinancialSummaryReportDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<DailyFinancialSummaryReportDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public DailyFinancialSummaryReportDto create(DailyFinancialSummaryReportDto dto) {
        ...
        DailyFinancialSummaryReport entity = DailyFinancialSummaryReport.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public DailyFinancialSummaryReportDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        DailyFinancialSummaryReport entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<DailyFinancialSummaryReportDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private DailyFinancialSummaryReportDto mapToDto(DailyFinancialSummaryReport e) {
        return DailyFinancialSummaryReportDto.builder()
            .id(e.getId())
