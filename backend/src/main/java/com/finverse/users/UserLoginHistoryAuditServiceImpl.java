...
/**
 * Transactional Service Implementation for UserLoginHistoryAudit
 */
@Service
@RequiredArgsConstructor
public class UserLoginHistoryAuditServiceImpl implements UserLoginHistoryAuditService {
    private final UserLoginHistoryAuditRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<UserLoginHistoryAuditDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<UserLoginHistoryAuditDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public UserLoginHistoryAuditDto create(UserLoginHistoryAuditDto dto) {
        ...
        UserLoginHistoryAudit entity = UserLoginHistoryAudit.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public UserLoginHistoryAuditDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        UserLoginHistoryAudit entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<UserLoginHistoryAuditDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private UserLoginHistoryAuditDto mapToDto(UserLoginHistoryAudit e) {
        return UserLoginHistoryAuditDto.builder()
            .id(e.getId())
