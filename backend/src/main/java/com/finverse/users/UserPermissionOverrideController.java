...
/**
 * REST API Controller for UserPermissionOverride
 */
@Tag(name = "Users - UserPermissionOverride")
@RestController
@RequestMapping("/api/v1/users/modules/userpermissionoverride")
...
public class UserPermissionOverrideController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<UserPermissionOverrideDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<UserPermissionOverrideDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<UserPermissionOverrideDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<UserPermissionOverrideDto>> create(@Valid @RequestBody UserPermissionOverrideDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<UserPermissionOverrideDto>> getByRef(@PathVariable String ref) {
