...
/**
 * Transactional Service Implementation for UserInvitationToken
 */
@Service
@RequiredArgsConstructor
public class UserInvitationTokenServiceImpl implements UserInvitationTokenService {
    private final UserInvitationTokenRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<UserInvitationTokenDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<UserInvitationTokenDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public UserInvitationTokenDto create(UserInvitationTokenDto dto) {
        ...
        UserInvitationToken entity = UserInvitationToken.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public UserInvitationTokenDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        UserInvitationToken entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<UserInvitationTokenDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private UserInvitationTokenDto mapToDto(UserInvitationToken e) {
        return UserInvitationTokenDto.builder()
            .id(e.getId())
