...
/**
 * Transactional Service Implementation for UserDelegationRule
 */
@Service
@RequiredArgsConstructor
public class UserDelegationRuleServiceImpl implements UserDelegationRuleService {
    private final UserDelegationRuleRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<UserDelegationRuleDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<UserDelegationRuleDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public UserDelegationRuleDto create(UserDelegationRuleDto dto) {
        ...
        UserDelegationRule entity = UserDelegationRule.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public UserDelegationRuleDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        UserDelegationRule entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<UserDelegationRuleDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private UserDelegationRuleDto mapToDto(UserDelegationRule e) {
        return UserDelegationRuleDto.builder()
            .id(e.getId())
