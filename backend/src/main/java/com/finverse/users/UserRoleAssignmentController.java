...
/**
 * REST API Controller for UserRoleAssignment
 */
@Tag(name = "Users - UserRoleAssignment")
@RestController
@RequestMapping("/api/v1/users/modules/userroleassignment")
...
public class UserRoleAssignmentController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<UserRoleAssignmentDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<UserRoleAssignmentDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<UserRoleAssignmentDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<UserRoleAssignmentDto>> create(@Valid @RequestBody UserRoleAssignmentDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<UserRoleAssignmentDto>> getByRef(@PathVariable String ref) {
