...
/**
 * REST API Controller for UserContactDirectory
 */
@Tag(name = "Users - UserContactDirectory")
@RestController
@RequestMapping("/api/v1/users/modules/usercontactdirectory")
@RequiredArgsConstructor
public class UserContactDirectoryController {
    private final UserContactDirectoryService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<UserContactDirectoryDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<UserContactDirectoryDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<UserContactDirectoryDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<UserContactDirectoryDto>> create(@Valid @RequestBody UserContactDirectoryDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<UserContactDirectoryDto>> getByRef(@PathVariable String ref) {
