...
/**
 * REST API Controller for UserSecuritySessionLog
 */
@Tag(name = "Users - UserSecuritySessionLog")
@RestController
@RequestMapping("/api/v1/users/modules/usersecuritysessionlog")
@RequiredArgsConstructor
public class UserSecuritySessionLogController {
    private final UserSecuritySessionLogService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<UserSecuritySessionLogDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<UserSecuritySessionLogDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<UserSecuritySessionLogDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<UserSecuritySessionLogDto>> create(@Valid @RequestBody UserSecuritySessionLogDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<UserSecuritySessionLogDto>> getByRef(@PathVariable String ref) {
