...
/**
 * Transactional Service Implementation for UserSecuritySessionLog
 */
@Service
@RequiredArgsConstructor
public class UserSecuritySessionLogServiceImpl implements UserSecuritySessionLogService {
    private final UserSecuritySessionLogRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<UserSecuritySessionLogDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<UserSecuritySessionLogDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public UserSecuritySessionLogDto create(UserSecuritySessionLogDto dto) {
        ...
        UserSecuritySessionLog entity = UserSecuritySessionLog.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public UserSecuritySessionLogDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        UserSecuritySessionLog entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<UserSecuritySessionLogDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private UserSecuritySessionLogDto mapToDto(UserSecuritySessionLog e) {
        return UserSecuritySessionLogDto.builder()
            .id(e.getId())
