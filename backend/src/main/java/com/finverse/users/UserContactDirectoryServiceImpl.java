...
/**
 * Transactional Service Implementation for UserContactDirectory
 */
@Service
@RequiredArgsConstructor
public class UserContactDirectoryServiceImpl implements UserContactDirectoryService {
    private final UserContactDirectoryRepository repository;
    private final UserRepository userRepository;

    @Override
    @Transactional(readOnly = true)
    public List<UserContactDirectoryDto> getByTenant(UUID tenantId) {
        return repository.findByTenantId(tenantId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<UserContactDirectoryDto> getByUser(UUID userId) {
        return repository.findByUserId(userId).stream().map(this::mapToDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public UserContactDirectoryDto create(UserContactDirectoryDto dto) {
        ...
        UserContactDirectory entity = UserContactDirectory.builder()
            .user(u)
    ...
    @Override
    @Transactional(readOnly = true)
    public UserContactDirectoryDto getByReference(String ref) {
    ...
    public void updateStatus(UUID id, String status) {
        UserContactDirectory entity = repository.findById(id)
    ...
    @Override
    @Transactional(readOnly = true)
    public List<UserContactDirectoryDto> listAll() {
        return repository.findAll().stream().map(this::mapToDto).collect(Collectors.toList());
    }

    private UserContactDirectoryDto mapToDto(UserContactDirectory e) {
        return UserContactDirectoryDto.builder()
            .id(e.getId())
