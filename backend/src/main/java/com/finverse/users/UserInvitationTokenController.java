...
/**
 * REST API Controller for UserInvitationToken
 */
@Tag(name = "Users - UserInvitationToken")
@RestController
@RequestMapping("/api/v1/users/modules/userinvitationtoken")
@RequiredArgsConstructor
public class UserInvitationTokenController {
    private final UserInvitationTokenService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<UserInvitationTokenDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<UserInvitationTokenDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<UserInvitationTokenDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<UserInvitationTokenDto>> create(@Valid @RequestBody UserInvitationTokenDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<UserInvitationTokenDto>> getByRef(@PathVariable String ref) {
