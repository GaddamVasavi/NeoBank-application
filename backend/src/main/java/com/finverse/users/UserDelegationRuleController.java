...
/**
 * REST API Controller for UserDelegationRule
 */
@Tag(name = "Users - UserDelegationRule")
@RestController
@RequestMapping("/api/v1/users/modules/userdelegationrule")
@RequiredArgsConstructor
public class UserDelegationRuleController {
    private final UserDelegationRuleService service;

    @Operation(summary = "List all entities")
    @GetMapping
    public ResponseEntity<ApiResponse<List<UserDelegationRuleDto>>> listAll() {
        return ResponseEntity.ok(ApiResponse.success(service.listAll()));
    }

    @Operation(summary = "List by tenant ID")
    @GetMapping("/tenant/{tenantId}")
    public ResponseEntity<ApiResponse<List<UserDelegationRuleDto>>> getByTenant(@PathVariable UUID tenantId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByTenant(tenantId)));
    }

    @Operation(summary = "List by user ID")
    @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<List<UserDelegationRuleDto>>> getByUser(@PathVariable UUID userId) {
        return ResponseEntity.ok(ApiResponse.success(service.getByUser(userId)));
    }

    @Operation(summary = "Create entity")
    @PostMapping
    public ResponseEntity<ApiResponse<UserDelegationRuleDto>> create(@Valid @RequestBody UserDelegationRuleDto dto) {
    ...
    @GetMapping("/ref/{ref}")
    public ResponseEntity<ApiResponse<UserDelegationRuleDto>> getByRef(@PathVariable String ref) {
